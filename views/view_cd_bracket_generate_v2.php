...
<form method="post" id="myform" action='<?php echo base_url(); ?>league_v2/bracket_save2' class="login-form" style="width:100%;"> 
...
$process_res = league_v2::cd_cal_c($pow_vals, $num_teams, $teams, $round, 'Main');
$teams = array();
...
$y = 1;
	 ...
	 $get_username = league_v2::get_username(intval($process_res[6][$ab][0]));
	 $get_partner		= league_v2::get_username(intval($double_bye_pl[1]));

	 $double_game_pl_2 = explode("-", $process_res[6][$ab][1]);
	 $get_username2 = league_v2::get_username(intval($process_res[6][$ab][1]));
	 $get_partner2	  = league_v2::get_username(intval($double_game_pl_2[1]));
...
$new_match_timings[$match_num] = array('1' => $court_name, '2' => $match_timings[$pick_time][2], '3' => $match_timings[$pick_time][3], '4' => date('m/d/Y H:i', $match_timings[$pick_time][2]), '5' => date('m/d/Y H:i', $match_timings[$pick_time][3]));
$pick_time++;
}
?>
...
//echo "<br>".$match_timings[$match_num][0] . " - " .date('m/d/Y H:i', (trim($match_timings[$match_num][2])));
	$i = 1;
	while($i <= 20 and ($new_match_timings[$prv_match1][2] == $match_timings[$pick_time][2] or $new_match_timings[$prv_match2][2] == $match_timings[$pick_time][2])) {
		$pick_time++;
		$i++;
	}
...
<?php
//echo "<pre>"; print_r($match_timings); print_r($new_match_timings);
//echo $first_round_matches;
//echo "<br>";
//echo $first_round_lastmatch_date;
//echo date('Y-m-d H:i', $first_round_lastmatch_date);
//exit;
...
$match_num = 1;
$cons_pick_time	   = 1;
$cons_new_match_timings = array();

?>
<link rel="stylesheet" href="<?php echo base_url();?>css/grids/cd/cr<?=$total_rounds;?>.css">

<div class="brackets" id="brackets">
<div style='margin-top:80px; margin-bottom:50px;'><?="<b>Consolation Draw</b>";?></div>
<div class="group<?php echo $total_rounds+1; ?>" id="b1">
<?php
for($round = 1, $source=1 ; $round <= $total_rounds; $round++) {
?>
...
$process_res = league_v2::cd_cal_c($pow_vals, $num_teams, $lteams, $round, 'Consolation');
$lteams = array();
...
$y = 1;
	...
	//$double_game_pl_2 = explode("-", $process_res[6][$ab][1]);
	 ...
	 //$get_partner2 = league_v2::get_username(intval($double_bye_pl[1]));
	$prv_match1 = '';
	$prv_match2 = '';
?>
...
$cons_match_dt_time = '';
$cons_court_name = '';
//echo "<pre>"; print_r($cons_match_timings);

if($cons_match_timings[$match_num]){
	$i = 1;
	while($i <= 20 and ($cons_new_match_timings[$prv_match1][2] == $cons_match_timings[$cons_pick_time][2] or $cons_new_match_timings[$prv_match2][2] == $cons_match_timings[$cons_pick_time][2] or $cons_match_timings[$cons_pick_time][2] < $first_round_lastmatch_date)) {
		$cons_pick_time++;
	$i++;
	}
			//echo "<pre>"; 
			//print_r($new_match_timings);
			//exit;
			$xy = 0;
			while($xy == 0){
				$yz = 0;
				foreach($new_match_timings as $mn_arr){
					if($cons_match_timings[$cons_pick_time][0] == $mn_arr[1] and ($cons_match_timings[$cons_pick_time][2] >= $mn_arr[2] and $cons_match_timings[$cons_pick_time][2] < $mn_arr[3])) {
						$cons_pick_time++;
						$yz = 1;
					}
				}
		...
		if($cons_new_match_timings[($match_num - 1)][1] == $cons_match_timings[$cons_pick_time][0])
			$cons_pick_time++;

			//echo $cons_pick_time;
$court_name = $cons_match_timings[$cons_pick_time][0];
//echo "&nbsp;&nbsp;&nbsp;".$court_name;
//echo "<pre> "; print_r($cons_match_timings[$cons_pick_time]); 
$cons_match_dt_time = date('m/d/Y H:i', (trim($cons_match_timings[$cons_pick_time][2])));
$cons_new_match_timings[$match_num] = array('1' => $court_name, '2' => $cons_match_timings[$cons_pick_time][2]);

$cons_pick_time++;
}
?>
<input type="text" id="cons_court_<?php echo $match_num; ?>" name="cons_assg_court<?php echo $match_num; ?>" 
value="<?php echo $court_name; ?>" style="width: 25%;" />
<input  type="text" class='' placeholder="Date" id="sdate_c<?php echo $match_num; ?>"  name="cmatch_date<?php echo $match_num; ?>" value="<?php echo $cons_match_dt_time; ?>" style="width: 50%;" />
</span>
...
<?php
	if($process_res[8][$ab][0]){
		...
		$get_username = league_v2::get_username(intval($process_res[8][$ab][0]));
		$get_dbl_partner1 = league_v2::get_username(intval($double_pl[1]));
...
<?php
	if($process_res[8][$ab][1]){
		...
		$get_username2	= league_v2::get_username(intval($process_res[8][$ab][1]));
		$get_dbl_partner2 = league_v2::get_username(intval($double_pl[1]));

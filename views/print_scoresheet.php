...
<br>
		<table class="score-cont" border= "0" width='80%' cellspacing = "3" cellpadding = "3" align="center">
			<tr>
				<td>#&nbsp;&nbsp;</td>
				<?php if($res_result1[0][0]->Match_DueDate != ""){ ?>
				<td>&nbsp;</td>
				<?php } ?>
				<td>Player</td>
				<td>Scores</td>
		...
		<?php $i = 1; 
			...
			<tr>
				<td><?=$i;?>&nbsp;&nbsp;</td>
				<?php
				if($drawresult->Match_DueDate or $drawresult->Court_Info){
				?>
				<td><?php
				if($drawresult->Court_Info) echo "<b style='background-color:#ffdc5b; padding:3px;'><i>".$drawresult->Court_Info."</i></b><br>";
				if($drawresult->Match_DueDate) {
					if(date('H:i', strtotime($drawresult->Match_DueDate)) != '00:00')
						echo date('M d, Y H:i', strtotime($drawresult->Match_DueDate));
					else
						echo date('M d, Y', strtotime($drawresult->Match_DueDate));
					}
				?>
				</td>
				<?php

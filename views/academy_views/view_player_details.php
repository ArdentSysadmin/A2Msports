...
<section id="single_player" class="secondary-page" style='padding:15px;'>
<div class='container'>
<div class='row'>
<div class="col-md-9">
<!-- Google AdSense -->
...
<?php $get_sport = player::get_sport($user_details['coach_sport']);
...
<form id="rate_form" method="POST" action="<?php echo $this->config->item('club_pr_url');?>/league/<?php if($get_userratings){
echo "EditRating";
...
$get_user = player::get_user($value->User_ID);
...
<td><?php
$get_opp_name = player::get_gen_mat_det($row->GeneralMatch_ID);

if($row->Opponent == $user_details['Users_ID'])
{
$get_username = player::get_user_det($get_opp_name['users_id']); 
echo $get_username['Firstname']." ".$get_username['Lastname'];
}
else
{
$get_username = player::get_user_det($row->Opponent); 
echo $get_username['Firstname']." ".$get_username['Lastname'];
}
...
<td><?php 
$get_username = player::get_user_det($row->Winner); 
echo $get_username['Firstname']." ".$get_username['Lastname'];
?></td>
</tr>
<?php } ?>
...
if($row->Player1_Score != "Bye Match" and $row->Player2_Score != "Bye Match"){
	$get_tourn = player::get_tourn_name($row->Tourn_ID); 
?>
<tr>
 <?php
//print_r($tour_ids);
...
{
$get_username = player::get_user_det($row->Player1); 
echo $get_username['Firstname']." ".$get_username['Lastname'];
if($row->Player1_Partner != 0)
{
$get_partner = player::get_user_det($row->Player1_Partner); 
echo "; ".$get_partner['Firstname']." ".$get_partner['Lastname'];
}
}
else
{
$get_username = player::get_user_det($row->Player2); 
echo $get_username['Firstname']." ".$get_username['Lastname'];
if($row->Player2_Partner != 0)
{
$get_partner = player::get_user_det($row->Player2_Partner); 
echo "; ".$get_partner['Firstname']." ".$get_partner['Lastname'];
}
}
...
if(count($user_stats) > 0){
	...
	$get_sport = player::get_sport($sport, $user_id);
	$get_a2m   = player::get_a2msocre($sport, $user_id);
	$get_winper   = player::get_winper($sport, $user_id);
...
<p>
<?php
$get_club = player::get_club($row->Club_id);
...
<?php $get_sport = player::get_sport($row->Related_Sport);
...
<form class="form-horizontal" id='myform' method='post' role="form"  action="<?php echo $this->config->item('club_pr_url');?>/search/contact_player"> 

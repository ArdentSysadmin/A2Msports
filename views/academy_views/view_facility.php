...
$(document).ready(function(){
	...
	$('#change_lt').click(function () {
		$('.lt_select_chck_imgs').toggle();
	});

	$('#del-lt-cancel').click(function () {
		$('.lt_select_chck_imgs').toggle();
	});

	$('#del-lt-imgs').click(function() {
		var academy_id  = "<?php echo $org_details['Aca_ID']; ?>";
		var short_code    = "<?php echo $org_details['Aca_URL_ShortCode']; ?>";
		var count			   = $('.lt_select_chck_imgs'). filter(':checked'). length;
		var sel_ps_imgs  = new Array();

        $( "input[name='sel_lt_imgs[]']:checked" ).each( function() {
                sel_ps_imgs.push( $( this ).val() );
        });

		var all_ps = $('#pre_lt_team').val();

		if(count){
			if(confirm('Are you sure to delete?')){
				$.ajax({
				//url: baseurl+'/'+short_code+'/facility/delete_glry',
				url: club_baseurl+'/facility/delete_lt',
				type: "post",
				data:{sel_ps:sel_ps_imgs, all_ps:all_ps},
				success: function(res) {
					//$('#response').html(data);
					alert('Selected Members are deleted!');
					location.reload();
				}
				});
			}
  ...
  <div class="container">
    <div class="title">
      ...
      <div><span></span></div>
			...
			&nbsp;&nbsp;&nbsp;
			<a style="color:#000; cursor:pointer; font-size:20px;" id="add_lt"><i class="fa fa-plus-circle" aria-hidden="true"></i></a>
			</div>
		<div class="col-md-12 col-sm-12 lt_select_chck_imgs" style='display:none;'>
		<input type='button' name='del-lt-imgs' id='del-lt-imgs' value=' Delete ' />
		<input type='button' name='del-lt-cancel' id='del-lt-cancel' value=' Cancel ' />
		</div>
			<?php } ?>
			<!-- edit icon end -->
    </div>
...
<div class='row justify-content-center' id='add-lead-team' style='display:none;'>
<form name='lt_add_frm' method='POST' action="<?=$this->config->item('club_form_url');?>/facility/add_lt_team" enctype='multipart/form-data'>
...
<div class="single-blog">
	...
	<input type='hidden' name='redirect_page' id='redirect_page' value='<?php echo $_SERVER['REQUEST_URI']; ?>' />
	<input type='hidden' name='pre_lt_team' id='pre_lt_team' value='<?php echo $facility_details['Facility_Leadership']; ?>' />
	<input type='submit' name='add_lt_submit' id='add_lt_submit' value='  Add  ' style='margin-right:15px;'/>
	<input type='button' name='add_lt_cancel' id='add_lt_cancel' value='  Cancel  '   />
	</div>

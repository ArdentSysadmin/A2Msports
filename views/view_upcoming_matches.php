...
$(document).ready(function(){
	...
	$('.btn_pay').on('click', function(e){
		...
		var fp    = $('#fp_'+i).val();
		var tid   = $('#tid_'+i).val();
		var rteam = $('#rteam_'+i).val();
		var mpaytm = $('#mpaytm_'+i).val();

		if(mpaytm == '' || mpaytm == 'NULL'){
		  $.ajax({
			type: 'POST',
			url: baseurl+'play/set_fee_sessions',
			data:{fp:fp, tid:tid, rteam:rteam},
			success: function(res) {
				//alert(res);
				$red_url = baseurl+'league/fee_pay/'+tid;
		  ...
		  $.ajax({
			...
			success: function(res) {
				//alert(res);
				$red_url = baseurl+'league/paytm_paynow/'+tid;
				$(location).attr('href', $red_url);
			}
		  });
		}
...
<?php 
$i = 0;
foreach($get_user_due_payments as $due_details){
$tr_age_group = json_decode($due_details->Age);
$tr_fee		  = json_decode($due_details->mult_fee_collect);
$key		  = array_search($due_details->Reg_Age_Group, $tr_age_group);
$fee_payable  = number_format($tr_fee[$key], 2);
$team_info	  = play::get_team($due_details->Team_id);

if($fee_payable != '0.00'){
?> 
<input type='hidden' name='fp'    id='fp_<?=$i;?>'	  value='<?=$fee_payable;?>' />
<input type='hidden' name='tid'   id='tid_<?=$i;?>'	  value='<?=$due_details->tournament_ID;?>' />
<input type='hidden' name='rteam' id='rteam_<?=$i;?>' value='<?=$due_details->Team_id;?>' />
<input type='hidden' name='mpaytm' id='mpaytm_<?=$i;?>' value='<?=$due_details->Merchant_Paytm;?>' />

<!-- <form name='tour_reg_fee_pay' method='POST' action='<?//=base_url();?>league/fee_pay/<?=$due_details->tournament_ID;?>'> -->
<div class="col-md-12 league-form-bg" style="
<?php if($i == 0) { echo "margin-top:40px;"; }else{ echo "margin-top:20px;"; } ?> background:#ffff8a; margin-bottom:5px">
<img src="<?=base_url();?>icons/warning_ico.png" width="30px" height="30px" border="0" alt="">&nbsp;<span style='font-size:13px;'>
Team <b><?=$team_info['Team_name'];?></b> is registered for <b><a href="<?php echo base_url().'league/'.$due_details->tournament_ID; ?>">
<?php echo $due_details->tournament_title; ?></a></b> to participate.&nbsp;&nbsp;&nbsp;</span>
<input type='submit' name='btn_pay' id='<?=$i;?>' value=' Pay Now ' class="league-form-submit btn_pay" style='font-size:12px; margin-bottom:0px; padding:8px 8px; float:right'/>
</div>
<!-- </form> -->
<?php
}
...
<div class="accordion"  id="up_match_section"  style="background:#f59123; padding:5px; color:white;"><i class="fa fa-arrow-circle-o-right" style="color:white;"> </i>Registered Tournaments/ Leagues<span></span></div>
...
<a href='<?php echo base_url(); ?>league/<?php echo $row->tournament_ID;?>'><b><?php echo $row->tournament_title;?></a></b>
</td>
...
<div class="accordion" id="up_match_section" style="background:#f59123; padding:5px; color:white;"><i class="fa fa-arrow-circle-o-right" style="color:white;"> </i>Upcoming Tournaments/ Leagues<span></span></div>
...
<?php
//echo count($up_event_tourns);
if(count($visible_tournments) == 0 and count($tournments) == 0 and count($up_event_tourns) == 0){
?>
<tr>
...
<?php echo base_url(); ?>tour_pictures/
<?php 
if($row->TournamentImage != "" and $row->TournamentImage != NULL ){ echo 'img'.$row->TournamentImage; }
else {
...
<?php if(($this->session->userdata('user')!="") && ($row->Usersid != $this->session->userdata('users_id'))){
	$now		=  strtotime("now");
	$oneday		= 86400;
	$reg_close	= strtotime($row->Registrationsclosedon);
	$reg_open	= strtotime($row->Registrations_Opens_on);

   if(($now >= $reg_open) and ($now <= $reg_close)){
	?>
   ...
   <?php 
	...
	?>
    <p style="color:red;">Registrations <br /> Opens on <?=date('m/d/Y',strtotime($row->Registrations_Opens_on));?></p>  
   <?php 
   }
   else{
   ?>
  <p style="color:red;">Registrations <br /> Closed</p>  
  <?php
	}
 } ?>
</div>
...
/* upcoming events */
foreach($up_event_tourns as $i => $event){ 
?>
<tr>
<td valign="center" align="center" ><?php echo date('m/d/Y',strtotime(substr($event->Ev_Start_Date,0,10))) ;?></td>
<td valign="center" align="center">
<p></p>
<div class="col-md-4" style="">
<img class="img-djoko" src="<?php echo base_url(); ?>tour_pictures/<?php if($event->EventImage!=""){echo $event->EventImage; }
  	else {
		switch($event->Ev_Sport) {
			case 1:
			echo "default_tennis.jpg";
			break;
			case 2:
...
<p style="font-size:16px"><a href="<?php
echo base_url().'events/'.$event->Ev_ID;
?>"><?php echo $event->Ev_Title;?></a></p>
<p><strong>Sport:</strong> <?php 
$get_sport = play::get_sport($event->Ev_Sport);
echo $get_sport['Sportname'];
?></p>
</td>
<td>&nbsp;</td>
...
<!-- end main body -->
<div class="col-md-12 league-form-bg" style="margin-top:40px; margin-bottom:20px;">
<div class="accordion" id="section3" style="background:#f59123; padding:5px; color:white;"><i class="fa fa-arrow-circle-o-right" style="color:white;"> </i>Past Tournaments/ Leagues<span></span></div>
...
?>
<tr><td colspan='3' style="padding-left:5px;"><h5>You were not registered for any tournament yet.</h5></td></tr>
<?php
}
else{
...
<td valign="center" align="center" ><?php echo date('m/d/Y',strtotime(substr($row->StartDate,0,10))) ;?></td>
<td valign="center" align="center">
<div class="col-md-3" style=""><img class="img-djoko" src="<?php echo base_url(); ?>tour_pictures/<?php if($row->TournamentImage!=""){
echo $row->TournamentImage; 
}
else{
	switch($row->SportsType){
		case 1:
...
?>" alt="" /></div>
<p style="font-size:16px"><a href="<?php echo base_url();?>league/<?php echo $row->tournament_ID;?>"><?php echo $row->tournament_title;?></a></p>
<p><strong>Sport:</strong> <?php 
$get_sport = play::get_sport($row->SportsType);
echo $get_sport['Sportname'];
?>
...
<td valign="center" align="center">
<div>
<?php if($row->users_id == $this->session->userdata('users_id')) {
?>
...
<div>
<?php
if(($row->users_id != $this->session->userdata('users_id'))){  
?>
<a href="<?php echo base_url();?>play/register/<?php echo $row->GeneralMatch_id;?>">Register</a>
<?php } ?>
</div>  
...
<div class="col-md-12 league-form-bg" style="margin-top:40px; margin-bottom:20px;">
<div class="accordion" id="section3" style="background:#f59123; padding:5px; color:white;">
<i class="fa fa-arrow-circle-o-right" style="color:white;"> </i>Past Matches<span></span></div>

...
<?php
$process_res = league_v2::cal_c($pow_vals, $num_teams, $teams, $round);
$teams = array();
...
if($this->input->post('tformat') == 'Teams' or $this->input->post('tformat') == 'TeamSport'){
	$team1 = league_v2::get_team(intval($process_res[6][$ab][0]));
	$team2 = league_v2::get_team(intval($process_res[6][$ab][1]));
...
$player2 = explode(";", $process_res[6][$ab][1]);
	...
	$player1 = league_v2::get_username(intval($rr_p1[0]));		
	$player2 = league_v2::get_username(intval($rr_p2[0]));
	
	$p1_part = "";		
	$p2_part = "";		
	if($rr_p1[1]){		
	$player1_partner = league_v2::get_username(intval($rr_p1[1]));		
	$p1_part = "; $player1_partner[Firstname] $player1_partner[Lastname]";		
	}		
	if($rr_p2[1]){		
	$player2_partner = league_v2::get_username(intval($rr_p2[1]));		

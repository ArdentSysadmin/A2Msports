...
//echo "<pre>"; print_r($event_reg_players); exit;
$grid_array = array();
?>
...
<input type='hidden' name='matches[]' value="<?php echo $match_num; ?>" />
	...
	<?php
		if($rr_matches[$m]->Court_Info != NULL and $rr_matches[$m]->Court_Info != ''){
			if($tour_details->Usersid == $this->logged_user or $this->is_super_admin){
				echo "<input type='text' name='match_court{$match_num}' value='{$rr_matches[$m]->Court_Info}' style='width:20%' />";
			}
			else{
...
<?php
		...
		if($get_name_partner){
			echo "&nbsp;<b>"."<a href='".$profile_base."player/".$get_name['Users_ID']."' target='_blank'>".$get_name['Firstname']." ".$get_name['Lastname']."</a>";
			echo "<br>";
			echo "&nbsp;<a href='".$profile_base."player/".$get_name_partner['Users_ID']."' target='_blank'>".$get_name_partner['Firstname'].".".$get_name_partner['Lastname']."</a></b>";
		}
		else{

	...
	if(format.indexOf('Doubles')!=-1){
                ...
                if(classname=='event_format'){
                    $.ajax({
						...
						data:{ tour_id:tour_id, events:events},
...
<div class="fromtitle">My Events for <?php echo $r->tournament_title; ?></div>
...
<?php  } ?>
<?php if($this->session->userdata('user')!="") { ?>
<form class="form-horizontal" id='myform' method='post' role="form" action="<?php echo base_url(); ?>league/buy_more/<?php echo $r->tournament_ID;?>" <?=$block_status;?>>
   ...
   $count_chekboxes = count($eligible_events);
	...
	// ----------------- Fetching Eligible Events -------
	 ...
	 if(count($regdata)>0){
		...
		echo "<label class='control-label col-md-4 padtop'  style='text-align: left; font-size:16px; margin-bottom: 10px;' for='id_accomodation'>";
		echo "<b> Registered Events</b>";
		echo "</label>";
	echo "<div class='mobile-tbl'>";
		echo "<div class='col-md-12 form-group internal text1'>
		<table cellpadding='15' style='padding:1px;'>";
		...
		foreach ($reg_events as $key => $value) {
			...
			echo " - ".$event."&nbsp;&nbsp;";
			echo "</td>";
			echo "<td><select class='user_event_change' name='change_to' id='change#to#{$key}'>";
			echo "<option value=''>Change To?</option>"; 
			if($count_chekboxes > 0){
			   foreach ($eligible_events as $key2 => $value) {
...
?>
        <div class="form-group" style="margin-top: 30px;">
           ...
           <label class='control-label padtop col-md-7' style="text-align: left; padding-right: 0px; font-size:15px;" for="">
		   <b>Select Partners?</b>&nbsp;&nbsp;&nbsp;</label>
			   &nbsp;&nbsp;&nbsp;  			  
			</div>
         </div>
...
<?php
	...
	echo "<div class='mobile-tbl'>";
		...
		foreach ($reg_events as $key => $value) {
		           ...
		           $users = league::in_array_r($evnt, $reg_users);
		           $cnt   = count($users);
		           $get_partner = league :: get_userPartner($this->logged_user, $evnt, $r->tournament_ID);

			//echo "<tr class='header' style='cursor:pointer;'><td>";
			echo "<tr><td>";
			//echo "<b id='chg_icon'>( + )</b>";
			//echo " ".$event." (".$cnt.")";
			echo " - ".$event."&nbsp;&nbsp;(".$get_partner.")&nbsp;&nbsp;";
			echo "</td>";
			echo "<td><select class='user_partner_change' name='partner_change_to' id='partner#change#to#{$key}'>";

			if($get_partner == 'No Partner Assigned')
				echo "<option value=''>Choose Partner</option>"; 
			else
				echo "<option value=''>Change Partner</option>"; 

            foreach($users as $user){
				if($user != $this->logged_user){
					$check_is_no_partner = league :: check_isUserNoPartner($user, $evnt, $r->tournament_ID);
					if($check_is_no_partner){
					$player	= league::get_username($user);
						if($get_partner != $player['Firstname']." ".$player['Lastname'])
						echo "<option value='".$player['Users_ID']."'>".$player['Firstname']." ".$player['Lastname']."</option>";
					}
				}
			}
			echo "</select>";
...
if($now < $reg_close or $this->logged_user == 237){
      ?>
        <div class="form-group" style="/*margin-top: 150px;*/">
			<div class='form-group internal text1' style="clear: both; padding-top: 30px;">
           <label class='control-label padtop col-md-7' style="text-align: left; padding-right: 0px; font-size:15px;" for="">
		   <b>Do you want to register more events?</b>&nbsp;&nbsp;&nbsp;</label>
	           <!-- <input name="multi_reg" type="radio" value="1" style="margin-bottom:22px"> Yes  
			   <input name="multi_reg" type="radio" value="0" checked> No -->  
	           <div><input id="multi_reg" name="multi_reg" type="button" class="form-control" value=" Click here " style="margin-top:10px; margin-bottom:22px; width:40%; color:#fff; background-color:#f59123;" /></div>
			   &nbsp;&nbsp;&nbsp;  
...
var baseurl = "<?php echo base_url();?>";
	...
	$('.user_event_change').change(function(){
		var temp = $(this).attr('id');
		var tempNew = temp.split('#');
		var prevEvent = tempNew[2];
		var changeTo = $(this).val();

		if(prevEvent && changeTo){
			if(confirm("Are you sure to change the event?")){
			var tid = $('#id').val();	
			$.ajax({		
				type:'POST',		
				url:baseurl+'league/change_user_reg_event/',		
				data:{tourn_id:tid,prevEvent:prevEvent,changeTo:changeTo},		
	...
	$('.user_partner_change').change(function(){
		var temp		= $(this).attr('id');
		var tempNew = temp.split('#');
		var event		= tempNew[3];
		...
		if(event && partner){
			if(confirm("Are you sure to choose this partner?")){
			var tid = $('#id').val();	
			$.ajax({		
				type:'POST',		
				url:baseurl+'league/change_user_event_partner/',		
				data:{tourn_id:tid,event:event,partner:partner},		
				success:function(res){
					alert(res);
					location.reload();
				}

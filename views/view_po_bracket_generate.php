...
<label>PO Draw Title*</label>&nbsp;&nbsp;<span id="title_error" style="color:red"></span>
...
$process_res = league::play_offs($pow_vals, $num_teams, $teams, $round, 'Main');
$teams = array();
...
$y = 1;
	...
	$double_bye_pl = explode("-", $process_res[6][$ab][0]);
	 ...
	 $team1_partner	= league::get_team(intval($double_bye_pl[1]));

	$double_game_pl_2 = explode("-", $process_res[6][$ab][1]);
	 $team2			 = league::get_team(intval($process_res[6][$ab][1]));
	 $team2_partner  = league::get_team(intval($double_game_pl_2[1]));
?>
...
<span class="teama">
<?php 
	if($team1){ echo $team1['Team_name']; }  else { echo "---"; }
	if($team1_partner){ echo " - ".$team1_partner['Team_name']; }
?></span>
<span class="teamb"><?php 
	if($team2){ echo $team2['Team_name']; } else { ($round==1)? $mn = "Bye": $mn = "---"; echo $mn; }
	if($team2_partner){ echo " - ".$team2_partner['Team_name']; }
?></span>
</div>
...
$process_res = league::play_offs($pow_vals, $num_teams, $lteams, $round, 'Consolation');
$lteams = array();
...
<?php
	if($process_res[8][$ab][0]){
		...
		$team1	  = league::get_team(intval($process_res[8][$ab][0]));
		$team1_partner = league::get_team(intval($double_pl[1]));
...
<span class="teamb"><?php
	if($process_res[8][$ab][1]){
		...
		$team2			= league::get_team(intval($process_res[8][$ab][1]));
		$team2_partner  = league::get_team(intval($double_pl[1]));

		if($team2){ echo $team2['Team_name']; }  else { echo "---"; }
		if($team2_partner){ echo "; ".$team2_partner['Team_name']; }
	}
	else{

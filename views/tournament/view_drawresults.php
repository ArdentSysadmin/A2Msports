...
var count=$('input:checkbox.delete_draws:checked').length;
	...
	$.ajax({
		type:'POST',
		url:baseurl+'league/publish_draws/',
...
var count=$('input:checkbox.delete_draws:checked').length;
	...
	$.ajax({
		type:'POST',
		url:baseurl+'league/unpublish_draws/',
		data:{ tourn_id:tourn_id, bracket_ids:bracket_ids },
		success:function(res){
	     location.reload();
		}
...
echo date('M d, Y',strtotime($get_first_match['Match_DueDate'])); 
}
else{
echo date('M d, Y',strtotime($tour_details->StartDate)); 
}
?></b></td>
<?php 
...
?>
<td>
<input type="button" name="Team_ScoreSheet_<?=$bk->BracketID;?>" id="<?=$bk->BracketID;?>" class="league-form-submit1 br_team_scoreSheet" value="ScoreSheet" onclick='myWin_scorecard(<?=$tour_details->tournament_ID;?>)' />
...
<?php
	...
	echo "<tr>";
	if($this->logged_user_role == 'Admin'){ 
	echo"<td>No Draws are available</td><td></td><td></td><td></td><td></td>";
	}
	else{
	echo "<td>No Draws are available</td><td></td><td></td>";
	}
	echo "</tr>";
}
...
$('#'+$bracket_id).val('Please wait....');
	...
	data:{bracket_id:$bracket_id,tformat:tourn_format,club_url:club_baseurl},
	success:function(res){

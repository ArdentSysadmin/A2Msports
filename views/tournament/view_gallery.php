...
$('body').on('click', '#upload_image', function(e){
        ...
        $.ajax({
            url: baseurl+'league/upload_pics',
            type: 'POST',
            xhr: function() {
...
?>
	...
	<?php
	if($this->logged_user_role == 'Admin' or
	$this->logged_user_role == 'RegPlayer' or
	($is_logged_user_reg and !array_key_exists($this->logged_user, $team_captains)) or
	array_key_exists($this->logged_user, $team_captains)	
	){
	?>
...
foreach($get_images as $i => $row){ ?>
		...
		<img src="<?php echo base_url(); ?>tour_pictures/<?php echo $row->Tournament_id; ?>/<?php if($row->Image_file!=""){ echo $row->Image_file; }

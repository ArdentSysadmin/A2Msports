	...
	$(document).ready(function(){
		...
		$('#myform').on('submit', function (e) {
			...
			if(isChecked){
				...
				var plrs_per_groups = count/sel_value;

				if(plrs_per_groups >= 3){
					var isPfChecked = $('#p_Yes').attr('checked');

					if(isPfChecked){
						var plof_size = $('#plof_size_sel').val();
						if(plrs_per_groups < plof_size){
							alert("Playoff escalation players are exceeds actual group players!");
							return false;
						}
						else{
							$('#plof_size').val(plof_size * sel_value);
							return true;
						}
					}
					else{
...
<div class='form-group' id='playoff_size' style='display:none'>
<label class='control-label col-md-5' style='padding-top:0px;'>Number of players to be moved to playoffs from each group?</label>
	<select class="form-control" name="plof_size_sel" id="plof_size_sel" style="width:10%">
		<option value='1'>1</option>
		<option value='2'>2</option>
		<option value='3'>3</option>
		<option value='4'>4</option>
	</select>
	<input type='hidden' name='plof_size' id='plof_size' value='' />
</div>
<div class='form-group' id='div_po_numsets' style='display:none;'>
<label class='control-label col-md-5' style='padding-top:0px;'>No. of Sets / Games per each match (Playoff)?</label>
<select class="form-control" name="po_num_of_sets" id="po_num_of_sets" style="width:10%">
<option value='1'>1</option>
...
$(document).ready(function() {
	...
	$(document).on('click', '#apply_to_courts', function() {
		//var sz = $("#court_sch_times .div_grp").size();
		var dt = $('#court_sch_times #mdate_0').val();
		var st = $('#court_sch_times #stime_0').val();
		var et = $('#court_sch_times #etime_0').val();
		...
		var ctrl2 = "<div class='col-md-2'>&nbsp;</div>";

		if (requested > count) {
			for (i = count; i < requested; i++) {

				if(i == 0)
				var ctrl2 = "<div class='col-md-2'><a id='apply_to_courts' style='cursor:pointer; text-decoration: none;'><b>Apply to all ?</b></a></div>";

				var $ctrl = $("<div class='col-md-12 div_grp' style='padding-bottom:4px;'><div class='col-md-3'><input type='text' class='form-control' name='courts[]' placeholder='Court Name/#"+(i+1)+"' value='Court "+(i+1)+"' /></div><div class='col-md-3'><input type='date' class='form-control ct_dt' name='match_date[]'  id='mdate_"+i+"' placeholder='MM/DD/YYYY' value='' max='' /></div><div class='col-md-2'><input type='time' class='form-control ct_st' name='stime[]' id='stime_"+i+"' value='' /></div><div class='col-md-2'><input type='time' class='form-control ct_et' name='etime[]' id='etime_"+i+"' value='' /></div>"+ctrl2+"</div>");

				$("#court_sch_times").append($ctrl);
				ctrl2 = "<div class='col-md-2'>&nbsp;</div>";

...
class paypal_lib {
	...
	function paypal_lib()
	{
		...
		$this->sandbox_paypal_url = SANDBOX_PAYPAL_URL;
	...
	function test_paypal_auto_form() 
	{
		// this function actually generates an entire HTML page consisting of
		// a form with hidden elements which is submitted to paypal via the 
		// BODY element's onLoad attribute.  We do this so that you can validate
		...
		//echo '<body style="text-align:center;" >' . "\n";
		echo '<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
		echo $this->test_paypal_form('paypal_auto_form');
		echo '</body></html>';
		exit;
	}

	function test_paypal_form($form_name='paypal_form') 
	{
		$str = '';
		$str .= '<form method="post" action="'.$this->sandbox_paypal_url.'" name="'.$form_name.'"/>' . "\n";
	...
	function paypal_api_form(){
		$str = '';
		$str .= '<html>' . "\n";
		$str .= '<head><title>Processing Payment...</title>' . "\n";
		$str .= '<meta name="viewport" content="width=device-width, initial-scale=1.0"></head>';
		$str .= '<body style="text-align:center;" onLoad="document.forms[\'paypal_auto_form\'].submit();">' . "\n";
		$str .= '<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
		//echo $this->paypal_form('paypal_auto_form');
		$str .= '<form method="post" action="'.$this->paypal_url.'" name="paypal_auto_form"/>' . "\n";
		foreach ($this->fields as $name => $value)
		$str .= form_hidden($name, $value) . "\n";
		//$str .= '<p>'. $this->submit_btn . '</p>';
		$str .= form_close() . "\n";
	...
	function sandbox_paypal_api_form(){
		$str = '';
		$str .= '<html>' . "\n";
		$str .= '<head><title>Processing Payment...</title>' . "\n";
		$str .= '<meta name="viewport" content="width=device-width, initial-scale=1.0"></head>';
		...
		$str .= '<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
		//echo $this->paypal_form('paypal_auto_form');
		$str .= '<form method="post" action="'.$this->sandbox_paypal_url.'" name="paypal_auto_form"/>' . "\n";
		foreach ($this->fields as $name => $value)
		$str .= form_hidden($name, $value) . "\n";
		//$str .= '<p>'. $this->submit_btn . '</p>';
		$str .= form_close() . "\n";

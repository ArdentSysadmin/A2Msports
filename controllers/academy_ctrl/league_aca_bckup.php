	...
	class League extends CI_Controller {
		...
		//$this->load->library('mypaypal');
			...
			$this->load->model('academy_mdl/model_league', 'model_league');
		...
		public function index($org_id = '')
		{
			...
			$this->load->view('academy_views/view_league',$data);
		...
		public function Sport_levels($sport_id = '')
		{
			...
			$sp_level = array();
			echo "<label class='control-label col-md-3' for='id_accomodation'>Levels </label>";
			foreach($sport_levels as $row){ 
			if(in_array($row->SportsLevel_ID, $sp_level)){ $checked_stat = 'checked = checked'; }
				
				echo "<input type='checkbox' class='' name='level[]' value='$row->SportsLevel_ID'>$row->SportsLevel";
				echo "&nbsp";echo "&nbsp";
			}
		}
		...
		public function create_trnmt()
	    {
			  $filename = 'TournamentImage';  
				...
				redirect($this->short_code."/league/view/$tourn_id");
		 }
		...
		public function view($tid)
		{
			...
			if($tr_det)
			{
				...
				if($this->logged_user){ $data['is_logged_user_reg'] = $this->model_league->is_logged_user_reg($tid); }
				else { $data['is_logged_user_reg'] = false; }
				...
				$data['is_logged_user_reg'] = $user_reg_team;
				 ...
				 if($user_reg_team and $team_fee_type == 'Player' and $tr_det->Usersid != $this->logged_user){
				...
				if(is_array($data['is_logged_user_reg']) or $data['is_logged_user_reg'] == '1'){
		...
		public function update_trnmt()
	    {
			...
			$data['tourn_id'] = $this->input->post('tourn_id'); 
			 ...
			 else 
			 {
				...
				redirect("academy_ctrl/league/view/$tourn_id");
		 }
		...
		public function get_reg_tourn_partner_names($tourn_id, $get_mf_type)
		{
			...
			if($tourn_id == "" and $get_mf_type == ""){

			$tourn_id	 = $this->input->post('tourn_id');
			$get_mf_type = $this->input->post('get_mf_type');
			
			$data['tourn_partner_names'] = $this->model_league->get_reg_tourn_partner_names($tourn_id, $get_mf_type);
			$data['get_mf_type']		 = $get_mf_type;

			$this->load->view('academy_views/view_dbl_partner_ajax',$data);
			}
			else
			{
				return $this->model_league->get_reg_tourn_partner_names($tourn_id, $get_mf_type);
			}
		 ...
		 public function get_tour_fee()
		 {
			...
			{
				if($sg === 'true' or $db === 'true' or $mx === 'true')
				{
					if($get_tour_info->is_mult_fee == 0)
					{
						
							$fee = number_format($get_tour_info->TournamentAmount, 2);
							if($sg === 'true' and ($db === 'true' or $mx === 'true')){
								$total_fee = $fee + number_format($get_tour_info->extrafee, 2);
							}
							else{
								$total_fee = $fee;
							}
					}
					else if($get_tour_info->is_mult_fee == 1)
					{
						$db_ag_grp				= json_decode($get_tour_info->Age);
						$mult_fee_collect		= json_decode($get_tour_info->mult_fee_collect);
						$addn_mult_fee_collect	= json_decode($get_tour_info->addn_mult_fee_collect);

						$index	= array_search($ag_grp, $db_ag_grp);
						$fee	= number_format($mult_fee_collect[$index], 2);
						$extra_fee	= number_format($addn_mult_fee_collect[$index], 2);

						/*if($sg === 'true' and ($db === 'true' or $mx === 'true')){
							$total_fee = $fee + number_format($addn_mult_fee_collect[$index], 2);
						}
						else{
							$total_fee = $fee;
						}*/
						...
						if($count == 1) { $total_fee = $fee; }
						else if($count == 2) { $total_fee = $fee + $extra_fee; }
						else if($count == 3) { $total_fee = $fee + $extra_fee + $extra_fee; }
					}
				}
				else if($get_tour_info->tournament_format == 'Teams' and $ag_grp){
		 ...
		 public function add_tourn_match_score($tourn_match_id)
		 {
			...
			if($res)
			{
				...
				$this->load->view($this->header_tpl);
				$this->load->view('academy_views/view_tournament',$data);
				$this->load->view($this->footer_tpl);
			}
		 }
		 ...
		 {	
			...
			if($details->tournament_format == 'Teams'){					
			...
			if($is_reg or 1){
				...
				else
				{
					...
					//$data['user_existed_teams'] = $this->general->get_user_existed_teams($tourn_id);		
		 ...
		 public function uprofile(){
			...
			if($redirect == 'league_view'){
				redirect($this->short_code."/league/view/$tour_id");
			}
			else{
				redirect($this->short_code."/league/register_match/$tour_id");
			}
		 }
		 ...
		 public function fixtures($tourn_id)
		 {
			...
			if($tour_det['Usersid'] == $this->logged_user)
			{
				...
				if(($match_type == 'Singles' or $match_type == '') and $tour_det['tournament_format'] == 'Individual'){
				...
				else if($tour_det['tournament_format'] == 'Teams'){		
				...
				$this->load->view($this->header_tpl, $data);
				$this->load->view('academy_views/view_fixtures',$data);
				$this->load->view($this->right_col_tpl,$data);
				$this->load->view($this->footer_tpl);
			}
			else{
				echo "Invalid/Unauthorised Access! Please contact admin@a2msports.com for any queries.";
			}
		 ...
		 public function age_group_users($tourn_id = '', $age_group = '', $match_type = '',$gender= '',$sport = '',$level = '')
		 {
			 ...
			 $tformat	 = $this->input->post('tformat');
				...
				$data['sport'] = $sport;
				$this->load->view('academy_views/view_agegroup_users' ,$data);
		 }
		 ...
		 public function registered_players()
		 {
			 ...
			 //if($age_type != ""){
					...
					$this->load->view('academy_views/view_reg_tourn_names',$data);
			 //}
		 }
		 ...
		 public function participants()
		 {
			 ...
			 if($age_type != ""){
					...
					$this->load->view('academy_views/view_tourn_participants',$data);
			}
		 }
		 ...
		 public function bracket($tourn_id = '', $org_id = '')
		 {
			...
			if($this->input->post('generate'))
			{
				...
				if($ttype == 'Single Elimination' or $ttype == 'Consolation')
				{
					...
					$this->load->view($this->header_tpl, $data);
					($ttype == 'Single Elimination') ? $this->load->view('academy_views/view_bracket_generate', $data) : $this->load->view('academy_views/view_cd_bracket_generate',$data);
					$this->load->view($this->footer_tpl);
				}
				else if($ttype == 'conventional' or $ttype == 'drive_chip_putt')
				{
					...
					$this->load->view($this->header_tpl, $data);
					$this->load->view('academy_views/view_golf_bracket_generate', $data);
					$this->load->view($this->footer_tpl);
				...
				else if($ttype == 'Round Robin')
				{
					...
					$this->load->view($this->header_tpl);
					$this->load->view('academy_views/view_rr_bracket_generate',$data);
					$this->load->view($this->footer_tpl);
				}
			}
		 }
		 ...
		 public function bracket_save($org_id = '')
		 {
			$tourn_id = $this->input->post('tourn_id');
		 ...
		 public function pdf($bracket_id)
		 {
			...
			else if($get_bracket['Bracket_Type'] == 'Round Robin')
			{
			 ...
			 $this->load->view('academy_views/view_se_print_draws', $data);
		 }
		...
		public function cd_cal_c($pow_vals, $num_teams, $teams, $round, $dt)		// Consolation
		{
		 ...
		 public function register_trnmt()
		 {
			...
			if($res)
			{
				redirect('academy_ctrl/play',$data);
			}
		 }

		 public function viewbracket($org_id = '', $tourn_id = '')
		 {
			...
			if($tourn_id)
			{
				...
				$this->load->view($this->header_tpl);
				$this->load->view('academy_views/view_tourn_bracket',$data);
				$this->load->view($this->footer_tpl);
			}
			...
			($this->input->post("template")=="view_tournament") ? 
				$this->load->view('academy_views/view_tournament', $data) : $this->load->view('academy_views/view_tourn_bracket', $data);
			$this->load->view($this->footer_tpl);
...
//print_r($data['get_tourn_matches']);
			...
			$html=$this->load->view('academy_views/view_bracket_print', $data, true);
		...
		public function RotateJpg($filename = '',$angle = 0,$savename = false,$tourn_id = '') {
				...
				case "image/gif": 
						...
						redirect($this->short_code."/league/view/$tourn_id","refresh");
		 ...
		 public function gallery()
		 {
			...
			$this->load->view($this->header_tpl);
			$this->load->view('academy_views/view_tournament_images',$data);
			$this->load->view($this->right_col_tpl,$data);
			$this->load->view($this->footer_tpl);
		 }
		 ...
		 public function images($tourn_id)
		 {
			...
			$this->load->view($this->header_tpl);
			$this->load->view('academy_views/view_tourn_individual_images',$data);
			$this->load->view($this->footer_tpl);
		 }
		 ...
		 public function get_gallery($tourn_id = '')
	     {
			...
			$this->load->view('academy_views/view_gallery_tournament',$data);
		 }
		 ...
		 public function double_players_change()
		 {
			...
			$data['get_mf_type']		 = $this->input->post('mf_filter');
		 ...
		 public function flyer($tid)
		 {
			...
			//echo "dfdsf ".$xyz;
			//exit;
			$this->load->view('academy_views/flyer',$data);
		 }

		 public function terms_conditions()
		 {	
			$this->load->view('academy_views/view_terms_conditions');
		 }

		 public function medical_form()
		 {	
			$this->load->view('academy_views/view_medical_release');
		 }
		 ...
		 public function fee_pay($tid)
		 {
			...
			if($this->session->userdata('tour_per_player_fee') > 0){
				...
				$returnURL = base_url().$this->short_code.'/paypal/pay_success?league='.$tid.'&team='.$reg_teamid.'&player='.$reg_userid; 
				//payment success url
				$cancelURL = base_url().$this->short_code.'/paypal/pay_cancel'; //payment cancel url
		 ...
		 public function buy($tid)
		 {
			...
			$returnURL = base_url().$this->short_code.'/paypal/success?tourn_id='.$tid.'&player='.$reg_userid.'&age_group='.$age_group.'&mtypes='.$match_types.'&partner1='.$db_partner.'&partner2='.$md_partner.'&level='.$level.'&hc_loc='.$hc_loc_id; //payment success url
			$cancelURL = base_url().$this->short_code.'/paypal/cancel'; //payment cancel url
			$notifyURL = base_url().'ipn'; //ipn url
			//get particular product data
			//$product = $this->model_league->getonerow($tid);
			//$userID = 1; //current user id
	...
	$ItemName 		= $this->logged_user; //Item Name
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
			...
			*/
			 ...
			 else if($this->input->post('tour_fee') == 0)   // Register without FEE functionality
			 {
				...
				if($tour_det->tournament_format == 'Teams'){						
					$res = $this->model_league->reg_team_tourn_no_fee();
				}
				else{
				...
				/*	$data['reg_suc'] = "You have successfully registered for this tournament.";
					...
					$data['results'] = $this->model_news->get_news();
				*/
						redirect($this->short_code."/league/view/$tourn_id/$reg_suc");
		...
		public function get_reg_team_players($tourn_id, $team_id)
		{
		 ...
		 public function get_reg_tourn_participants_withGender($tourn_id){
				...
				foreach($formats as $i => $fr){
					foreach($ag_group[$i] as $j => $ag){
						foreach($sp_levels[$i][$j] as $lv){
							...
							if($fr == 'Mixed'){ $gender_key = ''; }
      						$reg_users[$r->Users_ID][] = $gender_key.$fr."-".$ag."-".$lv;
						}
					}
				}

<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class League extends CI_Controller {
		...
		//$this->load->library('mypaypal');
			...
			$this->club_form_url = $this->config->item('club_pr_url')."/";

			if($this->session->userdata('users_id')){
				$this->logged_user = $this->session->userdata('users_id');
				if($this->session->userdata('email') == 'rajkamal.kosaraju@gmail.com' or $this->logged_user == 239 or $this->logged_user == 240){
			...
			$this->academy_admin	= $this->aca_general->get_org_admin($this->short_code);
			$this->academy_id		= $this->aca_general->get_orgid($this->short_code);
    ...
    /* ****************** Code to upload Details Rules & Medical Release Form Ends Here. *********************** */
		...
		public function view($tid, $stat = '')
		{
			...
			if($tr_det)
			{
				...
				if(($this->logged_user) 
					and ($tr_det->Usersid != $this->logged_user and $tr_det->Tournament_Director != $this->logged_user)){
				...
				$data['is_logged_user_reg'] = $user_reg_team;
				 ...
				 if(!empty($user_reg_team) 
					 and $team_fee_type == 'Player' 
					 and $tr_det->Usersid != $this->logged_user 
					 and $tr_det->Tournament_Director != $this->logged_user 
					 and $reg_closed_on >= $cur_date){
		...
		public function update_trnmt($org_id='')
			{
			//echo "test"; exit;
			//	echo "<pre>"; print_r($_POST); exit;
			// $sdate = date('h:i', strtotime($this->input->post('stime')));echo $sdate;exit();
				...
				$tourn_id=$data['tourn_id'];
				 ...
				 if($cpt>0){
					...
					redirect($this->config->item('club_pr_url')."/league/$tourn_id");
			 }
		...
		public function get_reg_tourn_partner_names($tourn_id, $event){
		 ...
		 public function apply_gc($gc = ''){
			...
			$tot_reg_fee	 = $this->input->post('tot_amt');
...
//exit;
			if($gc){
				...
				if($res['total_after_gc'] > -1){
					...
					$sess_data = array('tour_reg_fee' => number_format($res['total_after_gc'], 2));
...
//30.00#0#
		 ...
		 public function register_match($tourn_id, $org_id = '')
		{
			...
			if(!$this->logged_user){
				redirect($this->config->item('club_pr_url').'/login');
			}
			...
			if($is_reg or $now < $reg_close){
				...
				if($is_reg and !$details->Is_League)
				{
				...
				if($org_id)
					...
					if($details->Is_League)
						$this->load->view('academy_views/view_lg_register_match', $data);
					else
						$this->load->view('academy_views/view_register_match', $data); 
				...
				$age_array = array('U10','U11','U12','U13','U14','U15','U16','U17','U18');
				...
				if($org_id)
					...
					$this->load->view('academy_views/includes/view_right_column', $data);
					//$this->load->view('includes/view_right_column',$data);
		...
		public function uprofile($org_id = ''){
			//echo "<pre>"; print_r($_POST); echo $org_id; exit;
			...
			if($org_id){
				$aca_det = $this->general->get_aca_details($org_id);
				$short_code = $aca_det['Aca_URL_ShortCode'].'/';
			}
			if($redirect == 'league_view'){
				redirect($short_code."league/$tour_id");
			}
			else{
				redirect($short_code."league/register_match/$tour_id");
			}
		}

		 public function fixtures($tourn_id, $org_id)
		 {
			// echo $org_code." - ".$tourn_id;exit;
			//echo $tourn_id; exit;
			//print_r($this->session->userdata('user'));
			//exit;
		 	if(!$this->session->userdata('user')){ redirect('login'); }

			//$org_id					 = $this->aca_general->get_orgid($org_code);
			//$this->short_code = $org_code;
...
//echo "<pre>"; print_r($tour_det); exit;
		    if($user_id != $tour_det->Usersid 
				and $user_id != $tour_det->Tournament_Director 
				and !$this->is_super_admin){
               echo "Invalid Access!";
               exit();
		    }
	...
	//echo "<pre>"; print_r($data); 
			...
			$this->load->view('includes/footer');*/
					...
					$this->load->view('academy_views/includes/academy_league_header',$data);
					$this->load->view('academy_views/view_fixturestabs', $data); 
					$this->load->view('academy_views/includes/academy_right_column',$data);
					$this->load->view('academy_views/includes/academy_league_footer');	
		}
...
//echo "<pre>"; print_r($users_id);
		...
		public function getusers_02222022()
		{
		 	//print_r($_POST);exit();
		 	$format     = $this->input->post('format');
            $types      = $this->input->post('types');
           ...
           //echo "<pre>";print_r($users_id);exit();
			...
			$temp_arr = array();
			session_start();
			if($_SESSION['draw']['users']){
				$sess_draws_users = $_SESSION['draw']['users'];
				foreach ($users_id as $k => $value) {
					$xyz = explode('-', $value);
					if(!$xyz[1])
						$value = trim($value,'-'); // removing - if the value is no having partner but having - in the string
...
//echo "<pre>"; print_r($users_id);
			...
			// Keep the Seedings of Draw page when user comeback
//echo "<pre>"; print_r($users_id);
//echo "<pre>"; print_r($_SESSION['draw']['users']);
            foreach ($users_id as $key => $value) {
				$avg_a2m = '';
				$userArr = explode('-', $value);
			...
			//usort($users_list, "cmp_a2m");
			//echo "<pre>";print_r($_SESSION['draw']['users']);exit();
			if(!$_SESSION['draw']['users']){
			array_multisort(array_map(function($element) {return $element['A2M_MAX'];}, $users_list), SORT_DESC, $users_list);
			}
		...
		public function reg_teams($tourn_id)
		{
		 ...
		 public function get_reg_tourn_event_players_02222022($tourn_id, $types, $sport='', $is_checkin=''){
			   ...
			   foreach($res as $r){
					...
					if(!empty($user_reg_events)){
						foreach($user_reg_events as $event){
							...
							if (stripos(strtolower($event), 'Doubles') !== false) {
								if(!in_array($r->Users_ID, $double_partner_array[$event]) and !in_array($user_sel_partners[$event], $double_partner_array[$event])){
								$dbl_partner = $r->Users_ID.'-'.$user_sel_partners[$event];
								$reg_users[$dbl_partner][] = $event;
							...
							if (stripos(strtolower($event), 'Mixed') !== false) {
								if(!in_array($r->Users_ID, $mixed_partner_array[$event]) and !in_array($user_sel_partners[$event], $mixed_partner_array[$event])){
								$mx_partner = $r->Users_ID.'-'.$user_sel_partners[$event];
								$reg_users[$mx_partner][] = $event;
		 ...
		 public function bracket($tourn_id, $org_id)
		 {
			 //echo $org_code." ".$tourn_id; exit;
			//s echo "<pre>"; print_r($_POST); exit;
		 	if(!$this->session->userdata('user')){ redirect('login'); }
	...
	if($this->input->post('generate')){
		...
		//exit;
					...
					$this->load->view('academy_views/includes/academy_league_header',$data);
					$this->load->view('academy_views/view_bracket_groups_js', $data);
					$this->load->view('academy_views/view_bracket_groups', $data);
					$this->load->view('academy_views/includes/academy_league_footer');	
				}
				else{
...
print_r($bridge);*/
					...
					$this->load->view('academy_views/includes/academy_league_header',$data);
					$this->load->view('academy_views/view_sd_bracket_generate', $data);
				    $this->load->view('academy_views/includes/academy_league_footer');
				}
			  }
			}
	...
	if($total_matches > 3) {
		...
		$group_matches = 3;
				...
				exit;*/
				$this->load->view('academy_views/includes/academy_league_header', $data);
				$this->load->view('academy_views/view_rr_group_bracket_generate', $data);
				$this->load->view('academy_views/includes/academy_league_footer');
				}
...
//					$data['players'] = array(A,B,C,D,E,F,G,H);
				...
				$sd_matches = array();
					...
					$this->load->view('academy_views/includes/academy_league_header', $data);
					$this->load->view('academy_views/view_sd_group_bracket_generate', $data);
				    $this->load->view('academy_views/includes/academy_league_footer');
				}
			}
		 }
		 ...
		 public function bracket_save($org_code)
		 {	
			...
			$tourn_id = $this->input->post('tourn_id');
//echo "<pre>"; echo $this->club_form_url; exit;
			if($this->input->post('rr_bracket_confirm')){
				...
				$res = $this->model_league->insert_group_sd_brackets(); // draw_format done
			}
				...
				$res = $this->model_league->insert_sd_brackets(); // draw_format done
			}
			else if($this->input->post('po_auto_esc_upd') and $this->input->post('upd_type') == 'AUTO_ESC'){
				$res = $this->model_league->update_po_autoesc();
			}
			
			if($res){
				 redirect($this->club_form_url."league/".$tourn_id);
			}
		 }
		...
		*/
			...
			$this->load->view('includes/header');
				...
				$tour_dir		= $data['tourn_det']->Tournament_Director;
				$sport			= $data['tourn_det']->SportsType;

				if($tour_admin != $this->logged_user and $tour_dir != $this->logged_user and $this->logged_user){
					$brackets = $this->get_bracket_list($tid);
					if(count(array_filter($brackets)) > 0){
						foreach($brackets as $bk)
						{
				...
				$data['is_logged_user_reg'] = $user_reg_team;
				 ...
				 if(!empty($user_reg_team) and $team_fee_type == 'Player' and $tr_det->Usersid != $this->logged_user and $tr_det->Tournament_Director != $this->logged_user){
...
/* **************** TeamSport ******************** */
			...
			if($this->input->post("list_draw_matches".$bid)){
				...
				if($tour_admin != $this->logged_user){
					$brackets = $this->get_bracket_list($tid);
					if(count(array_filter($brackets)) > 0){
						foreach($brackets as $bk)
						{
		 ...
		 public function email_add_score($res)
		 {
			...
			$tourn_dir		 = $tourn_det->Tournament_Director;
			$sess_id		 = $this->logged_user;

			if($sess_id != $tourn_admin and $sess_id != $tourn_dir){
...
document.f1.submit();
		 ...
		 public function buy($tid, $org_id = '')
		 {
			 //echo "<pre>"; print_r($_POST);exit();
			$tour_det  = $this->model_league->getonerow($tid);
			...
			if($this->input->post('tour_fee') == '' and $tour_det->Tournamentfee == 1){
			 ...
			 if($this->input->post('tour_fee') != $this->session->userdata('tour_reg_fee') and ($tid == $this->session->userdata('reg_tour_id')) and $this->input->post('tour_fee') == ''){
				echo "Sorry, something went wrong! Please contact <a href='mailto:admin@a2msports.com'> Admin </a>. Thank you";
				exit;
			 }
			...
			if($tour_det->Merchant_Paytm != '' 
				and $tour_det->Merchant_Paytm != NULL 
				and $this->input->post('tour_fee') > 0) {

				//echo $this->input->post('tour_fee'); exit;
				$this->test_func($tid);
				exit;
			}
			...
			if($this->input->post('tour_fee') > 0){
				...
				if($tour_det->tournament_format == 'Teams' || $tour_det->tournament_format == 'TeamSport'){
					...
					if($this->config->item('club_form_url') == ''){
						...
						$ret_site = $this->config->item('club_pr_url').'/';
					}

					$returnURL = $ret_site.'paypal/success?tourn_id='.$tid.'&team='.$reg_team.'&reg_user='.$reg_user.'&team_players='.$reg_team_players.'&age_group='.$age_group.'&level='.$level.'&hc_loc='.$hc_loc_id.'&ttype='.$ttype.'&tsize='.$tsize.'&coup_code='.$coup_code.'&coup_disc='.$coup_disc.'&nte_to_admin='.$note_to_admin; 
					//payment success url

					$cancelURL = $ret_site.'paypal/cancel'; //payment cancel url
					$notifyURL   = $ret_site.'paypal/ipn';	 //ipn url
				}
				else{
					$sess_data = array();
					...
					$returnURL = $ret_site.'paypal/success?tourn_id='.$tid.'&player='.$reg_userid.'&age_group='.$age_group.'&mtypes='.$match_types.'&partners='.$partners.'&level='.$level.'&hc_loc='.$hc_loc_id.'&ttype='.$ttype.'&tsize='.$tsize.'&coup_code='.$coup_code.'&coup_disc='.$coup_disc.'&events='.$reg_events.'&occr='.$occr_json.'&nte_to_admin='.$note_to_admin.'&school_info='.$school_info;  
					//payment success url
					//echo $returnURL;
					//exit;
			...
			$tourn_events_limit=json_decode($tour_det->Event_Reg_Limit,true);
				...
				foreach ($tourn_events_limit as $event => $limit) {
					...
					$this->session->unset_userdata('reg_tour_id');
						...
						redirect($this->config->item('club_pr_url')."/league/$tourn_id/$reg_suc");	
				}
			 }
			 else{
				echo "Invalid Request!";
		 ...
		 public function buy_more($org_code, $tid)
		 {
//echo $this->input->post('tour_fee')."<br>";
//echo $this->session->userdata('tour_reg_fee')."<br>";
//exit;
//echo $tid;
//exit;
			...
			if($this->input->post('tour_fee') > 0){
					...
					$returnURL = $this->club_form_url.'paypal/msuccess?tourn_id='.$tid.'&player='.$reg_userid.'&age_group='.$age_group.'&mtypes='.$match_types.'&level='.$level.'&ttype='.$ttype.'&events='.$reg_events; 					
					//echo $this->input->post('tour_fee');
					//echo $returnURL;exit();
					//payment success url

					$cancelURL = $this->club_form_url.'paypal/cancel'; //payment cancel url
					$notifyURL = $this->club_form_url.'paypal/ipn_more';	 //ipn url
				

			//$logo = $this->club_form_url.'images/logo.png';
		...
		public function get_matches_cnt($user_id, $sport_id)
		{
		 ...
		 public function delete_draws()
		 {
            if(isset($_POST['bracket_ids']))
			{
				...
				$get_tour_admin = $this->model_league->getonerow($tourn_id);
				//echo $get_tour_admin->SportsType;exit();
				if($get_tour_admin->Usersid == $this->logged_user or $get_tour_admin->Tournament_Director == $this->logged_user or $this->is_super_admin){
		 ...
		 public function Getteamplayers()
		 {
            ...
            echo "<div class='col-md-3 namespadd'><a target='_blank' href='".$this->config->item('club_form_url')."player/".$usr."'>".$user_det['Firstname'].' '.$user_det['Lastname']."</a></div>";
            }    
			exit();
		 }
		 ...
		 public function get_event_fee(){
		   ...
		   if($coupon != ''){
				$res = $this->calc_discount_amt($coupon, $fee, $tour_id);
			...
			$this->session->set_userdata($sess_data);
//echo "test ".$this->session->userdata('tour_reg_fee');
//exit;
...
//exit;
		...
		public function get_tour_details(){
			...
			foreach ($tourn_events_limit as $event => $limit) {
			            ...
			            $evnt=$gen.' '.$eventarr[2].' '.$level_name;
...
//}
		...
		public function sendmail_tm_participants($short_code){
...
//exit;
            ...
            /*if(!empty($_FILES['attach_file']['name'])){		
				...
				// Mail without attachment 
			     ...
			     $data['players']  = $player_ids_jsn;
			     $data['is_academy']  = 1;
			     $data['academy_id']  = $this->academy_id;
			     //echo "<pre>"; print_r($data); exit;
				 $res = $this->model_league->insert_sendmail_tmparticipants($data);
				 echo $res;
			}
...
print_r($mult_events);*/
		...
		public function viewtournament($tid, $org_id = '', $stat = '')
		{
			unset($_SESSION['draw']);
			...
			if($tr_det)
			{
				...
				if($this->logged_user and ($tr_det->Usersid == $this->logged_user or $tr_det->Tournament_Director == $this->logged_user or $this->is_super_admin)){    /// tournament admin access links
					$this->logged_user_role = 'Admin';
					//echo $this->logged_user; exit;
				}
				else if($is_reg){
					$this->logged_user_role = 'RegPlayer';
				}
...
//echo $this->logged_user_role; exit;
				...
				if($data['udob'] == '0' or $data['uaddr'] == '0' or $data['uemail'] == '0'){
					$data['r']		    = $tr_det;
					$data['results'] = $this->model_news->get_news();
					$this->load->view('academy_views/tournament/view_update_basic_profile', $data); 
					$this->load->view('academy_views/includes/academy_right_column',$data);
				}
				else{
					$data['show_landing_page']	= 1;
					...
					$data['is_reg_game_day'] = 0;
					$data['is_a_game_day']    = 0;
					$data['reg_occr'] = 0;
					$data['lg_occr'] = 0;

				if($tr_det->Is_League and $this->logged_user){
					$reg_occr = $this->model_league->get_user_reg_occrs($tid, $this->logged_user);
					$data['reg_occr'] = $reg_occr;
					$reg_occr_list = array();
					foreach($reg_occr as $occr){
						$reg_occr_list[] = $occr->OCCR_ID;
					}

					$lg_occr	= $this->model_league->get_league_occr($tid);
					$today		= date('Y-m-d');
					foreach($lg_occr as $ocr){
						//if($this->logged_user == 237)
							//echo $today." ".date('Y-m-d', strtotime($ocr->Game_Date))." $ocr->OCR_ID<br>";
						if($today == date('Y-m-d', strtotime($ocr->Game_Date)) and in_array($ocr->OCR_ID, $reg_occr_list)){
							$data['is_reg_game_day'] = 1;
							$data['is_a_game_day']    = 1;
							$data['game_day_dt']    = date('Y-m-d H:i', strtotime($ocr->Game_Date));
						}
						//$ocr->OCR_ID, $ocr->Game_Date);
					}
					//echo "<pre>"; print_r($ev_occr);
					$data['lg_occr'] = $lg_occr;

					//if($this->logged_user == 237)
						//echo "<pre>"; print_r($data); exit;
				}
//echo $this->logged_user_role; exit;
					$this->load->view('academy_views/view_tournament_new', $data);
				}
		...
		public function ShowMatches($org_code, $bracket_id){
...
//echo "<pre>"; print_r($data); exit;
		 ...
		 public function register_more($org_code, $tourn_id)
		 {
			...
			// echo "test ".$club_sh_code." ".$tourn_id; exit;
			//$tourn_id	 = $this->uri->segment(3);
            //echo $tourn_id;
			$org_id = $this->aca_general->get_orgid($org_code);
$this->short_code = $org_code;
			...
			$data['act_menu_list'] = $this->model_academy->get_act_menu_list($org_id);
					...
					if($details->Is_League){
						...
						$this->load->view('academy_views/includes/academy_league_header',$data);
						$this->load->view('academy_views/view_lg_register_more', $data); 
						$this->load->view('academy_views/includes/academy_right_column',$data);
						$this->load->view('academy_views/includes/academy_league_footer');	
					}
					else{
						$this->load->view('academy_views/includes/academy_league_header',$data);
						$this->load->view('academy_views/view_register_more_matches', $data); 
						$this->load->view('academy_views/includes/academy_right_column',$data);
						$this->load->view('academy_views/includes/academy_league_footer');
					}
				}
		...
		public function load_tourn_players($tid){
			$tid	= $this->input->post('tid');
			$tr_det = $this->get_logged_user_role($tid);
			$this->is_team_league = ($tr_det->tournament_format  == 'Teams' || $tr_det->tournament_format  == 'TeamSport') ? 1 : 0;
			...
			echo $this->load->view('tournament/view_players', $data);
		}
		...
		public function edit($tourn_id, $org_id = ''){
			...
			if($org_id){
			 ...
			 if($tournament_details->Usersid == $user_id or $tournament_details->Tournament_Director == $user_id or $this->is_super_admin){
				...
				if($tournament_details->Is_League == 1)
					$this->load->view('academy_views/view_edit_league', $data);
				else
					$this->load->view('academy_views/view_edit_tournament', $data);
...
// when player swap b/w group, need to place him based on the rating.
		...
		//print_r($groups);
			...
			echo $this->load->view('academy_views/view_bracket_groups', $data);
		}
...
/* **************** */
		...
		public function printScoreSheet(){
			...
			$this->load->view('academy_views/print_scoresheet', $data);
		}
		...
		public function user_usatt_update(){
			...
			$this->model_league->user_usatt_update();
			$tour_id = $this->input->post('txt_tid');
	...
	public function getEventLocation($loc_id){
		...
		public function reg_players($tourn_id, $org_id = '')
		{
			//error_reporting(-1);
			$data['org_url_key'] = "";
			...
			if(!$this->input->post('bulk_register'))
			{
				...
				if($details->Usersid == $this->session->userdata('users_id') 
					...
					$data['r'] = $details;
					$data['results'] = $this->model_news->get_news();
					$data['optimum_users'] = $this->model_league->get_tourbased_users($tourn_id);
...
$sno++;
		...
		public function check_in($short_code=''){
			...
			//echo "<pre>"; echo 'test '.$this->config->item('proxy_url'); exit;

			//redirect($this->config->item('proxy_url').'/league/'.$tourn_id);
			echo "Successfully Checked In. Thank you";
		}

		public function set_bracket_session() {
			$sess_data													= array();
			$sess_data['admin']['draw']['format']				= $this->input->post('draw_format');
			...
			//$this->session->set_userdata($sess_data); // This will add the session info to existing userdata
			session_start();
			unset($_SESSION['draw']);
			$_SESSION['draw'] = $sess_data['draw'];
			 if($this->logged_user == 240){
				//redirect('https://playgpa.com/test/sess_test');

				//echo "<pre>New Session values<br>";
		...
		public function unpublish($short_code, $tourn_id) {
			$this->get_logged_user_role($tourn_id);
		...
		public function publish($short_code, $tourn_id) {
			$this->get_logged_user_role($tourn_id);
	...
	public function fetch_topGroup_players(){
		$tourn_id		= $data['tid'] = $this->input->post('tid');
		$bracket_id  = $data['bid'] = $this->input->post('bid');
		$ref_draws  = $this->input->post('ref');
		$esc_count  = $this->input->post('esc');

		if(!$ref_draws){
			echo "Can't fetch the players as no main draws exists!";
			exit;
		}

		if($tourn_id and $bracket_id) {

			$ref_draws = json_decode($ref_draws, TRUE);
			foreach($ref_draws as $rf) {
				 $is_complete = $this->model_league->is_draw_complete($rf);
				
				if($is_complete) {
					echo "Source draws are not yet completed!";
					exit;
				}
			}
$esc_players = array();
			foreach($ref_draws as $rf) {
						$esc_players	= $this->evaluate_top_winners($rf, $esc_count, $esc_players);
						//$esc_players[$rf]		= $fetch_top_players;
						//$esc_players[$rf]		= $fetch_top_players;
			}

			//echo "<pre>"; print_r($esc_players); 
array_multisort(array_map(function($element) { 
					return $element['wp']; }, $esc_players), SORT_DESC, $esc_players);

			//echo "<pre>"; print_r($data); exit;
			$data['esc_players'] = $esc_players;
			$this->load->view('academy_views/view_esc_fixturestabs', $data);
		}
		else {
			echo "Invalid Access!";
			exit;
	...
	public function evaluate_top_winners($bracket_id, $esc_count, $esc_players) {

				  /*$num_rounds = intval($bk->No_of_rounds-1);*/
				  $rr_tourn_matches = $this->get_tourn_rr_matches($bracket_id);
				  $players = array();
...
$players = $temp_players_sort;
			//echo "<pre>"; print_r($players); exit;
				...
				foreach($players as $pl => $x){
					...
					if($i == 1 and $x['points'] > 0){
						$place1 = $ply[0];
						$place1_partner = $ply[1];
						$place1_wp = $x['winper2'];
					}
					if($i == 2 and $x['points'] > 0){
						$place2 = $ply[0];
						$place2_partner = $ply[1];
						$place2_wp = $x['winper2'];
					}
					if($i == 3 and $x['points'] > 0){
						$place3 = $ply[0];
						$place3_partner = $ply[1];
						$place3_wp = $x['winper2'];
					}
					if($i == 4 and $x['points'] > 0){
						$place4 = $ply[0];
						$place4_partner = $ply[1];
						$place4_wp = $x['winper2'];
					}
					if($i == 5 and $x['points'] > 0){
						$place5 = $ply[0];
						$place5_partner = $ply[1];
						$place5_wp = $x['winper2'];
					}
	...
	//$temp = array();
		//echo $place1.$place2.$place3.$place4; exit;
		for($k = 1; $k <= $esc_count; $k++) {
			if(${"place".$k."_partner"})
				$pl = ${"place".$k}."-".${"place".$k."_partner"};
			else
				$pl = ${"place".$k};

			$esc_players[] = array('pl' => $pl, 'wp' => ${"place".$k."_wp"});
			//$temp[] = ${"place".$k};
		}
...
//echo "<pre>"; print_r($_POST); exit;
		...
		<script>
			...
			document.getElementById('po_auto_esc_upd').click();
			}
		</script>
		</head>
		<body style="text-align:center;">
		<p style="text-align:center;">Please wait... Playoff matches are being updated.</p>
		<?php
					//$data['types']		  = $this->input->post('types');
					//$data['type_format']  = $this->input->post('type_gen');
					$tourn_id	  = $this->input->post('tourn_id');
					$teams		  = $this->input->post('users');
					$bid		  = $this->input->post('bid');
					$upd_type		  = $this->input->post('upd_type');
					$num_of_teams = count($teams);
...
?>
		<form method="post" id="myform" action='<?php echo $this->club_form_url; ?>league/bracket_save' class="login-form" style="width:100%; display:none;">
...
<input type="submit" class="league-form-submit1" name="po_auto_esc_upd" id="po_auto_esc_upd" value="Confirm & Save" />
</div>
...
<input type="hidden" name="upd_type"  id="upd_type"  value='<?=$upd_type;?>' />
<input type="hidden" name="bracket_id"  id="bracket_id"  value='<?=$bid;?>' />
...
<?php
	...
	$host = base_url()."rest_services/messages";

    $ch = curl_init();
  
    $dataArray = array('type' => 'tournament', 'type_id' => $tourn_id, 'is_web'	=> 1);

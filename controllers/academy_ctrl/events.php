...
class events extends CI_Controller {
	...
	//echo "<pre>"; print_r($data); exit;
		...
		$data['club_events']		= $this->model_academy->get_user_create_events($data['creator']);

		$data['results']				= $this->model_news->get_news();
	//echo "<pre>"; print_r($data); exit;

		$this->load->view($this->header_tpl, $data);
		$this->load->view('academy_views/view_league_list',  $data);
		//$this->load->view('includes/view_right_column',$data);
		$this->load->view($this->footer_tpl);
	}

	public function gpaevents($org_id)
	{
		//$data = $this->get_org_details($org_id);
		$org_details			 = $this->model_academy->get_academy_details($org_id);
		$data['org_details']	 = $org_details; 
	...
	//	$data['club_leagues']	= $this->model_academy->get_club_tournments($data['creator']); 
		...
		$this->load->view($this->header_tpl, $data);
		$this->load->view('academy_views/view_custom_leagues1', $data);
		$this->load->view($this->footer_tpl);
	}

	public function add_league($org_id){
		//echo "<pre>";
		//print_r($_POST);
		$txt = $this->input->post('txt-editor');
		$data = './static_pages/gpa_leagues.php';	// it is the path of the text files 

		//$urls = file_put_contents($data, $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
		//echo $txt;
		//exit;
		$urls = file_put_contents($data, $txt);
		redirect('/testclub9/gpaleagues');
	}
	...
	public function send_event($short_code = '')
	{
		...
		$adm_msg = $this->input->post('ev_message');
		...
		foreach($sel_players as $user_id)
		{
			...
			if($stat){
				...
				$subject = "Invitation for Event - ".$ev_title;
		...
		redirect($this->short_code."/events/$event_id/2");
		}
		else{
		echo "Invalid Access!";
		}
...
//exit;
	...
	public function view($short_code, $ev_id, $act_code = "")
	{
		$data = $this->get_org_details($this->academy_id);
		...
		$data['ev_det']			 = $ev_det;
		//print_r($data);
		$data['results']			 = $this->model_news->get_news($this->academy_id);
	...
	public function invite_players($short_code = '')
	{
				$aca_info		= $this->model_academy->get_academy_details($this->academy_id);
				$aca_name = $aca_info['Aca_name'];
		...
		foreach($explode as $to_email){
			...
			$this->email->subject( "New Event (".$ev_title.") Invitation.");
...
//echo "<pre>"; print_r($_POST); print_r($data); exit;
	...
	public function reg_user2($short_code = ''){
		//print_r($_POST); exit;
		$users_id = $this->session->userdata('users_id');
		if(!$users_id){ echo "Invalid Request!"; exit; }
...
//echo "<pre>"; print_r($_POST); print_r($data); exit;
		...
		if(!$is_ev_user){
			...
			redirect($this->config->item('club_pr_url')."/events/$event_id/6");
			}
		}
		else{
			redirect($this->config->item('club_pr_url')."/events/$event_id/5");
		}
	}

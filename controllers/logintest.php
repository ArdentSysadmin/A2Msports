...
$redirect_uri = 'https://a2msports.com/logintest/auth_check';
...
$this->load->view('view_login2', $data);
...
//echo $phone." , ".$ph;
			...
			if($validate_user){
				...
				$res['res'][] = array('user_id' => $user_det['Users_ID'], 'firstname' => $user_det['Firstname'], 
					'lastname' => $user_det['Lastname'], 'email' => $user_det['EmailID']);
				}
				if(count($res['res']) == 1){
					//$this->create_session($user_det['Users_ID'], 1);
				}
			}
			else{
				$res['status'] = "Phone Number Not Registered!";
				$res['mobile'] = $phone;
			}
		}
		else{
		...
		public function create_session($user, $is_valid){
			if($is_valid == '1'){

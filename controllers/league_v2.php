<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	session_start(); 
	//League controller ..
	class League_v2 extends CI_Controller {
...
//30.00#0#
	...
	//echo "<pre>"; print_r($data); 
		...
		$data['courts_new']		= "";
		$data['match_timings']	= "";
		$data['main_draw_dt']	= $main_draw_dt = $this->input->post('main_draw_dt');

		if($this->input->post('is_sch_courts')){
			$main_draw_dt = strtotime($main_draw_dt);

			$num_courts		= $this->input->post('num_courts');
			$num_cons_courts		= $this->input->post('num_cons_courts');
			$crts					= $this->input->post('courts');
			$cc_crts					= $this->input->post('cc_courts');
			$match_dates	= $this->input->post('match_date');
			$cc_match_dates	= $this->input->post('cc_match_date');
			$start_time			= $this->input->post('stime');
			$cc_start_time			= $this->input->post('cc_stime');
			$end_time			= $this->input->post('etime');
			$cc_end_time			= $this->input->post('cc_etime');

			$duration				= $this->input->post('match_duration');
			$break					= $this->input->post('match_break');

			$cons_duration = $this->input->post('cons_match_duration');
			$cons_break     = $this->input->post('cons_match_break');

			$tot_match_duration			  = $duration + $break;
			$cons_tot_match_duration = $cons_duration + $cons_break;
					...
					if($main_draw_dt == ''){
						$data['main_draw_dt']	= $main_draw_dt = strtotime($match_dates[$i] . " " . $start_time[$i]);
					}
					...
					foreach($cc_crts as $i => $court){

					$start  = strtotime($cc_match_dates[$i] . " " . $cc_start_time[$i]);
					$end   = strtotime($cc_match_dates[$i] . " " . $cc_end_time[$i]);

					$cons_currentdate	= $start;
					$cons_time_arr		= array();

					while($cons_currentdate <= $end){
					  $cons_time_arr[]		= $cons_currentdate;
					  $cons_currentdate	= strtotime('+'.$cons_tot_match_duration.' minutes', $cons_currentdate);
					}

						$cons_courts_new[$i] = array('name'	=> $court, 
																'timings'	=> array('sd' => $cc_match_dates[$i],
																						  'st'  => $cc_start_time[$i],
																						  'ed' => $cc_end_time[$i]
																						),
																'time_breaks'	=> $cons_time_arr,
																'duration'			=> $this->input->post('cons_match_duration'),
																'break'				=> $this->input->post('cons_match_break')
																);
					...
					for($m = 1; $m < 180; $m++){
						...
						if($main_draw_dt and $courts_new[$crt]['time_breaks'][$tm_index] > $main_draw_dt){
							$main_ed_tm = strtotime('+'.$tot_match_duration.' minutes', $courts_new[$crt]['time_breaks'][$tm_index]);
						$match_timings[$q] = array($courts_new[$crt]['name'], 
																		$courts_new[$crt]['timings']['sd'], 
																		$courts_new[$crt]['time_breaks'][$tm_index],
																		$main_ed_tm
																	 );
						$q++;
						}
			...
			if($cons_duration){
					...
					for($m = 1; $m < 180; $m++){
						...
						if($main_draw_dt and $cons_courts_new[$crt]['time_breaks'][$tm_index] > $main_draw_dt){
							$ed_tm = strtotime('+'.$cons_tot_match_duration.' minutes', $cons_courts_new[$crt]['time_breaks'][$tm_index]);
						$cons_match_timings[$p] = array($cons_courts_new[$crt]['name'], 
																				$cons_courts_new[$crt]['timings']['sd'], 
																				$cons_courts_new[$crt]['time_breaks'][$tm_index],
																				$ed_tm
																				);
						$p++;
						}

						if($crt == ($num_cons_courts - 1))
							$crt = 0;
						else
							$crt++;

						if(($m%$num_cons_courts) == 0){
							$tm_index++;
						}
					}
			}
					...
					$data['cons_courts_new']		= $cons_courts_new;
					$data['cons_match_timings']	= $cons_match_timings;
		...
		//exit;
				...
				if($ttype == 'Single Elimination' or $ttype == 'Consolation'){
					...
					$this->load->view('includes/header');
					($ttype == 'Single Elimination') ? 
						$this->load->view('view_bracket_generate_v2', $data) : $this->load->view('view_cd_bracket_generate_v2', $data);
					$this->load->view('includes/footer');
				}
		...
		public function cal_c($pow_vals, $num_teams, $teams, $round)			// Single Elimination
		{
			...
			$res = array();
				...
				if($round == 1 and $num_teams != 2){
					...
					$res[5] = league_v2::getBracket($teams);
				}
		 ...
		 public function buy($tid)
		 {
			...
			if($this->input->post('tour_fee') > 0){
				...
				if($tour_det->tournament_format == 'Teams' || $tour_det->tournament_format == 'TeamSport'){
					...
					$returnURL = base_url().'paypal/success?tourn_id='.$tid.'&player='.$reg_userid.'&age_group='.$age_group.'&mtypes='.$match_types.'&partners='.$partners.'&level='.$level.'&hc_loc='.$hc_loc_id.'&ttype='.$ttype.'&tsize='.$tsize.'&coup_code='.$coup_code.'&coup_disc='.$coup_disc.'&events='.$reg_events.'&nte_to_admin='.$note_to_admin.'&school_info='.$school_info;  
					//payment success url
					//echo $returnURL;
					//exit;
					$cancelURL = base_url().'paypal/cancel'; //payment cancel url
   ...
   return $result; 
			...
			foreach($get_lines as $m => $line){
				...
				if($line->Match_Type == 'MDoubles' or $line->Match_Type == 'WDoubles' or $line->Match_Type == 'Mixed' or $line->Match_Type == 'OSingles' or $line->Match_Type == 'ODoubles'){
					if($line->Player1_Partner){
						$get_user		 = league_v2::get_username($line->Player1_Partner);
						$player1_partner	  = "; <a href='".base_url()."player/".$get_user['Users_ID']."' title=''>"
						.$get_user['Firstname']." ".$get_user['Lastname']."</a>";
					}
					if($line->Player1_Partner){
						$get_user		 = league_v2::get_username($line->Player2_Partner);
		...
		/*public function in_array_r($needle, $haystack, $strict = false){
			foreach($haystack as $i => $item){
				if(($strict ? $item === $needle : $item == $needle) || (is_array($item) && league_v2::in_array_r($needle, $item, $strict))){
					//echo $i;
					return $i;
				}
			}
		...
		public function get_matches_cnt($user_id, $sport_id)
		{
		 ...
		 public function Getteamplayers()
		 {
            ...
            foreach ($team_players as $key => $usr) {
                 $user_det = league_v2::get_username($usr);
		 ...
		 public function get_event_fee(){
		   ...
		   //echo "<pre>";
		   //echo $get_tour_info->Event_Reg_Fee;
		   //print_r($events);
...
//exit;
        ...
        public function array_flatten($array) { 
			  ...
			  foreach ($array as $key => $value) { 
			    if (is_array($value)) { 
			      $result = array_merge($result, league_v2::array_flatten($value)); 
			    } 
			    else { 
			      $result[$key] = $value; 
			    } 
...
print_r($mult_events);*/
			...
			foreach($mult_events as $key => $val){
				if(is_numeric($key)){
				   ...
				   if( $arr[1] != 'Open')
				   $get_level_label = league_v2::get_level_name('', $level);
//echo $ag[0].' '.$level;
					if($ag == 'Adults')
...
/* **************** */
		...
		function getBracket($participants)
		{
			...
			for($round=1; $round < $rounds; $round++)
			{
				...
				{
					$home = league_v2::changeIntoBye($match[0], $participantsCount);
					$away = league_v2::changeIntoBye($sum - $match[0], $participantsCount);
					$roundMatches[] = array($home, $away);
					$home = league_v2::changeIntoBye($sum - $match[1], $participantsCount);
					$away = league_v2::changeIntoBye($match[1], $participantsCount);
					$roundMatches[] = array($home, $away);
				}
		...
		public function edit_user_a2m(){
			...
			if($user){
				...
				if($this->logged_user == $tour_details->Usersid or $this->is_super_admin){
					$upd = $this->model_league->update_user_a2m($user, $tour_details->SportsType, $up_a2m);

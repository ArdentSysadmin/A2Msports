...
class Payments_pro extends CI_Controller 
{
	...
	function Player_withdraw(){
		...
		$totevents = $this->input->post('totevents');
		$ref_amt	= $this->input->post('refund_amt');
		$ref_info	 = $this->input->post('ref_info');

		$ref_info1	 = explode('_', trim($this->input->post('ref_info')));

		$tourn_id	 = $ref_info1[0];
		$users_id	 = $ref_info1[1];
		$regtourn_id = $ref_info1[2];

		$trans_ids	= json_decode($this->input->post('trans_id'), true);

		
		$bal_amt = $ref_amt;

			foreach($trans_ids as $txn_id => $txn_amt){
				if($bal_amt > 0){
					if($ref_amt == $txn_amt){
						$player_refund = $this->Player_Refund_transaction($txn_id, $ref_amt, 'Full');
						$bal_amt = 0;
					}
					else if($ref_amt < $txn_amt){
						$player_refund = $this->Player_Refund_transaction($txn_id, $ref_amt, 'Partial');
						$bal_amt = 0;
					}
					else if($ref_amt > $txn_amt){
						$bal_amt = $ref_amt - $txn_amt;
						$ref_amt  = $txn_amt;
						$player_refund = $this->Player_Refund_transaction($txn_id, $ref_amt, 'Full');
					}
				}
			...
			if($player_refund){
				$upd = $this->unregister_player_from_tourn_with_refund($player_refund, $ref_info, $sel_events, $totevents);
			}
			else{
				$upd = $this->unregister_player_from_tourn_no_refund($ref_info, $sel_events, $totevents);
			}

			echo true;
	}

	function unregister_player_from_tourn_no_refund($r, $sel_events, $totevents){

				$ref_info	 = explode('_', trim($r));
				$tourn_id	 = $ref_info[0];
				$users_id	 = $ref_info[1];
				$regtourn_id = $ref_info[2];
				$withdrawn_events    = $sel_events;
				$user_updated_events = json_decode($totevents);

				foreach($user_updated_events as $i => $event){
					if(in_array($event, $withdrawn_events)){
						unset($user_updated_events[$i]);
					}
				}
			//print_r($user_updated_events);
			//exit;

			$data1 = array(
					'Tournament_id'			=> $tourn_id,
					'Users_ID'				=> $users_id,
					'RegisterTournament_id' => $regtourn_id
					);

			if(!empty($user_updated_events)){
			
			$data = $this->Get_Events($user_updated_events);
            $reg_events = json_encode($user_updated_events);
			/*$data2 = array(
					'Refund_Init'=> 0,
			...
			$data2 = array(
					...
					$ref_success = $this->model_league->upd_withdraw_status($data1, $data2);
					$tourn_res   = $this->model_league->getonerow($tourn_id);
					$user_res    = $this->model_league->get_user_name($users_id);

						$subject = "Player Withdraw - {$tourn_res->tournament_title}";

						$data = array(
							'player'		   => $user_res['Firstname']." ".$user_res['Lastname'],
							'tourn_id'		   => $tourn_id,
							'withdrawn_events' => $withdrawn_events,
							'title'			   => $tourn_res->tournament_title,
							'page'			   => 'WithDraw without Refund partial events by Player'
							);
					...
					$ref_success = $this->model_league->del_tour_reg_user($data1);
					$tourn_res   = $this->model_league->getonerow($tourn_id);
					$user_res    = $this->model_league->get_user_name($users_id);
						...
						$data = array(
							...
							'tourn_id'	=> $tourn_id,
							'title'		=> $tourn_res->tournament_title,
							'page'		=> 'WithDraw without Refund all events by Player'
							);
	...
	function unregister_player_from_tourn_with_refund($PayPalResult, $r, $sel_events, $totevents){
			...
			$data3 = array(
					...
					'Tournament_ID' => $tourn_id,
					'Users_ID'	 => $users_id,
					'Ref_Date'	 => date('Y-m-d H:i:s'),  //$PayPalResult['TIMESTAMP']
					'Ref_Type'	 => $PayPalResult['REQUESTDATA']['REFUNDTYPE'],
					'Ref_Amt'	 => number_format((float)$PayPalResult['GROSSREFUNDAMT'], 2),
					'Ref_TxnID'  => $PayPalResult['REFUNDTRANSACTIONID'],
					'Ref_Status' => $PayPalResult['ACK']
					);

					$ref_success = $this->model_league->upd_refund_status($data1, $data2, $data3);
					$tourn_res = $this->model_league->getonerow($tourn_id);
					$user_res  = $this->model_league->get_user_name($users_id);

						$subject = "Registration Withdraw by Player - {$tourn_res->tournament_title}";
						...
						$data = array(
							...
							'Ref_Date'		   => date('Y-m-d H:i:s'), // $PayPalResult['TIMESTAMP']
							'Ref_Type'		   => $PayPalResult['REQUESTDATA']['REFUNDTYPE'],
							'Ref_Amt'		   => number_format($PayPalResult['GROSSREFUNDAMT'], 2),
							'Ref_TxnID'		   => $PayPalResult['REFUNDTRANSACTIONID'],
							'page'			   => 'WithDraw with Refund partial events by Player'
							);
					...
					$user_res    = $this->model_league->get_user_name($users_id);
						...
						$data = array(
							...
							'Ref_Amt'	=> number_format($PayPalResult['GROSSREFUNDAMT'], 2),
							'Ref_TxnID' => $PayPalResult['REFUNDTRANSACTIONID'],
							'page'		=> 'WithDraw with Refund all events by Player'
							);
	...
	function Player_Refund_transaction($trans_id, $refund_amt, $refund_type){

		$RTFields = array(
			'transactionid' => $trans_id, 					// Required.  PayPal transaction ID for the order you're refunding.
			...
			'amt' => number_format($refund_amt, 2), 		// Refund Amt.  Required if refund type is Partial.  
			'currencycode' => $currencycode, 				// Three-letter currency code.  Required for Partial Refunds.  Do not use for full refunds.
		...
		if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])){
			$data['errors'] = array('Errors' => $PayPalResult['ERRORS']);
			$this->load->view('view_paypal_errors', $data);
		}
		else{
				return $PayPalResult;
		}
	...
	function Refund_transaction()
	{
		...
		//$refund_type  = trim($this->input->post('refund_type'));
		...
		else
		{
			...
			exit;*/
				$ref_info	 = explode('_', trim($this->input->post('ref_info')));
				$tourn_id	 = $ref_info[0];
				$users_id	 = $ref_info[1];
				$regtourn_id = $ref_info[2];

				$withdrawn_events	 = $this->input->post('match_type_'.$users_id);
				$user_updated_events = json_decode($this->input->post('user_db_events_'.$users_id));
			...
			$data3 = array(
					...
					$user_res  = $this->model_league->get_user_name($users_id);
						...
						$subject = "Registration Withdraw by Admin - {$tourn_res->tournament_title}";
						...
						$data = array(
							...
							'Ref_Amt'		   => number_format($PayPalResult['GROSSREFUNDAMT'], 2),
							'Ref_TxnID'		   => $PayPalResult['REFUNDTRANSACTIONID'],
							'page'			   => 'WithDraw with Refund partial events by Admin'
							);
			...
			print_r($user_updated_events);*/
				...
				$data3 = array(
					...
					$ref_success2 = $this->model_league->upd_refund_status2($data1, $data3);
					$ref_success = $this->model_league->del_tour_reg_user($data1);
					$tourn_res   = $this->model_league->getonerow($tourn_id);
					$user_res    = $this->model_league->get_user_name($users_id);
						...
						$data = array(
							...
							'Ref_Amt'	=> number_format($PayPalResult['GROSSREFUNDAMT'], 2),
							'Ref_TxnID' => $PayPalResult['REFUNDTRANSACTIONID'],
							'page'		=> 'WithDraw with Refund all events by Admin'
							);
			...
			$data2 = array(
					...
					$user_res    = $this->model_league->get_user_name($users_id);
						...
						$data = array(
							...
							'withdrawn_events' => $withdrawn_events,
							'title'			   => $tourn_res->tournament_title,
							'page'			   => 'WithDraw without Refund partial events by Admin'
							);
					...
					$user_res    = $this->model_league->get_user_name($users_id);
						...
						$data = array(
							...
							'tourn_id'	=> $tourn_id,
							'title'		=> $tourn_res->tournament_title,
							'page'		=> 'WithDraw without Refund all events by Admin'
							);
	...
	{
		$GTDFields = array(
			'transactionid' => '7W7343492L568062K'	
						// PayPal transaction ID of the order you want to get details for.
						);

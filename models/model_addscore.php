...
*/
			...
			if($p1_a2mscore_updated < 75){ $p1_a2mscore_updated = 75; }
			if($p2_a2mscore_updated < 75){ $p2_a2mscore_updated = 75; }
...
*/
				...
				if($p1_a2mscore_updated < 75){ $p1_a2mscore_updated = 75; }
				if($p1_part_a2mscore_updated < 75){ $p1_part_a2mscore_updated = 75; }
				if($p2_a2mscore_updated < 75){ $p2_a2mscore_updated = 75; }
				if($p2_part_a2mscore_updated < 75){ $p2_part_a2mscore_updated = 75; }
			...
			$a2mscore_upd_qry1 = $this->db->update('A2MScore', $data);
				...
				$data = array ('A2MScore' => $p1_part_a2mscore_updated);
			...
			$a2mscore_upd_qry3 = $this->db->update('A2MScore', $data);
				...
				$data = array ('A2MScore' => $p2_part_a2mscore_updated);
		...
		public function get_user_tournaments(){
			...
			$query = $this->db->query("SELECT * FROM RegisterTournament as rt JOIN tournament as t on rt.Tournament_ID	= t.tournament_ID 
			WHERE rt.Users_ID = {$user_id} AND t.EndDate > '{$today}'");
		...
		public function get_brackets($bracket_id)   // Get Brackets to show matches in AddScore (TopMenu)
		{
			...
			$qry_check = $this->db->query("SELECT * FROM Tournament_Matches WHERE (Player1 = $user_id OR Player2 = $user_id OR Player1_Partner = $user_id OR Player2_Partner = $user_id ) AND BracketID = $bracket_id AND (Winner = 0 OR Winner IS NULL)");

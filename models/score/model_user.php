...
class model_user extends CI_Model {
	...
	public function validate_user($email, $pwd) {
		$pwd = md5($pwd);
		$qry_check = $this->db->query("SELECT * FROM Users WHERE (EmailID = '".$email."' OR UserProfileName = '".$email."' ) AND Password = '".$pwd."'");
		return $qry_check->row_array();
	}
...
if($search['city'] or $search['state']){
	...
	if($search['latitude'] and $search['longitude']){
		...
		if($club_id != '' and $is_sort and $sport != ''){			
			$qry_check = $this->db->query("SELECT (SELECT MAX(MaxValue) FROM (VALUES (a.A2MScore),(a.A2MScore_Doubles),(a.A2MScore_Mixed)) AS Value(MaxValue)) AS A2MScore, u.* FROM Users u JOIN A2MScore a ON u.Users_ID = a.Users_ID AND a.SportsType_ID = '".$sport."' WHERE u.Users_ID IN (SELECT Users_id FROM User_memberships WHERE Club_id = {$club_id}){$search_cond} ORDER BY A2MScore {$is_sort} {$offset_qry}");

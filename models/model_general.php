<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Model_general extends CI_Model {
		...
		public function get_user($uid){
			...
			//echo ""; print_r($query->row_array()); exit;
			return $query->row_array();
		}
		...
		public function get_player($uid){
			...
			$query = $this->db->get_where('Users',$data);
			$p = $query->row_array();
			$name = "<a href='".base_url()."player/{$p['Users_ID']}'>".ucfirst($p['Firstname'])." ".ucfirst($p['Lastname'])."</a>";
			return $name;
		}
		...
		public function get_player_partner($uid, $pid){
			...
			$pp			= $query2->row_array();
			$name		= "<a href='".base_url()."player/{$p['Users_ID']}'>".ucfirst($p['Firstname'])." ".ucfirst($p['Lastname'])."</a>; "."<a href='".base_url()."player/{$pp['Users_ID']}'>".ucfirst($pp['Firstname'])." ".ucfirst($pp['Lastname'])."</a>";
			return $name;
		}
		...
		public function get_bracket_firstmatch($bid){
			$query = $this->db->query("select * from Tournament_Matches WHERE BracketID = {$bid} and Match_Num = 1");
			//echo $this->db->last_query(); exit;
			return $query->row_array();
		}

...
*/
   ...
   AS A2MScore
		...
		public function get_TeamsByCountry2($country, $sport){
			...
			$qry_check = $this->db->query("SELECT TOP(20) u.Users_ID, u.Firstname,u.Lastname,t.Team_name,t.Captain,t.Created_by,t.Players,t.Team_ID,t.Team_Logo,h.hcl_city,h.hcl_state,a.A2MTeamScore FROM Users u JOIN Teams t ON u.Users_ID = t.Created_by AND t.Sport = '".$sport."' JOIN A2MTeamScore a ON a.Team_ID = t.Team_ID 
			JOIN Home_Court_Locations h ON t.Home_loc_id = h.hcl_id AND {$qry_str} ORDER BY a.A2MTeamScore DESC");
...
//echo $this->db->last_query();
	...
	public function get_pb_defRating($user_id, $tourn_id, $reg_events){
		...
		$checkPb = $this->db->query("SELECT * FROM RegisterTournament WHERE Users_ID = {$user_id} AND Tournament_ID IN (SELECT tournament_ID FROM tournament WHERE tournament_ID != {$tourn_id} AND SportsType = 7)");

...
class model_club extends CI_Model {
	...
	public function get_limit_clubs($user_id='', $page, $limit){
		...
		if($user_id and $det['Country']){
			$query = $this->db->query("SELECT * FROM Academy_Info WHERE Aca_country = '{$det['Country']}' OFFSET {$offset} ROWS FETCH NEXT {$limit} ROWS ONLY");

			$qry_total = $this->db->query("SELECT * FROM Academy_Info WHERE Aca_country = '{$det['Country']}' OFFSET {$offset} ROWS FETCH NEXT {$limit} ROWS ONLY");
		}
		else{
			$query	   = $this->db->query("SELECT * FROM Academy_Info OFFSET {$offset} ROWS FETCH NEXT {$limit} ROWS ONLY");
			$qry_total = $this->db->query("SELECT * FROM Academy_Info OFFSET {$offset} ROWS FETCH NEXT {$limit} ROWS ONLY");
		}

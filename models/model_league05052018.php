	...
	class model_league extends CI_Model {
		...
		*/
		...
		public function get_reg_team_tourn_participants($tourn_id){
			$qry_check = $this->db->query("SELECT * FROM RegisterTournament WHERE Tournament_ID = $tourn_id AND Users_ID IS NULL");
			return $qry_check->result();
		}
...
* COS( RADIANS( $lat )) * COS( RADIANS( Longitude ) - RADIANS( $long )) ) * 3964.3 AS distance FROM tournament WHERE ACOS( SIN( RADIANS( Latitude )) * SIN( RADIANS( $lat ) ) + COS( RADIANS( Latitude ) ) * COS( RADIANS( $lat )) * COS( RADIANS( Longitude ) - RADIANS( $long )) ) * 3964.3 < {$range} AND Usersid = $users_id ");
		...
		public function reg_tourn_with_fee($data){
			...
			$get_tourn_reg = $this->db->query("SELECT * FROM RegisterTournament WHERE Tournament_ID = {$data['tourn_id']} AND Users_ID={$data['player']}");
	...
	/*	public function update_tourn_dates()
		{
			...
			if($r1_match_count){
				...
				for($i=1; $i<=$r1_match_count; $i++){
					...
					if(($player1 == $player2) or ($player1 == '0' and $player2 == '0') or ($player1 == '' or $player2 == '')){
						$invalid = 1;
					}
					$p1_array[] = $player1;
					$p2_array[] = $player2;
						...
						for($i=1; $i<=$r1_match_count; $i++){
							...
							if($winner != 0){
								...
								if($qry_check->num_rows() > 0)
								{
									...
									if($winner_dest['Player1_source'] == $i){
										$qry_check = $this->db->query("UPDATE Tournament_Matches SET Player1 = $winner WHERE Tourn_match_id = $winner_dest[Tourn_match_id]");
																		//echo $this->db->last_query();
									}
									else if($winner_dest['Player2_source'] == $i){
										$qry_check = $this->db->query("UPDATE Tournament_Matches SET Player2 = $winner WHERE Tourn_match_id = $winner_dest[Tourn_match_id]");
																		//echo $this->db->last_query();
									}
								}
							...
							else
							{
								...
								if($qry_check->num_rows() > 0)
								{
									...
									if($winner_dest['Player1_source'] == $i){
										$qry_check = $this->db->query("UPDATE Tournament_Matches SET Player1 = 0 WHERE Tourn_match_id = $winner_dest[Tourn_match_id]");
																		//echo $this->db->last_query();
									}
									else if($winner_dest['Player2_source'] == $i){
										$qry_check = $this->db->query("UPDATE Tournament_Matches SET Player2 = 0 WHERE Tourn_match_id = $winner_dest[Tourn_match_id]");
																		//echo $this->db->last_query();
									}
								}
							}
...
//echo $qry_check_p2->num_rows(); exit;
			($qry_check_p2->num_rows() > 0) ? $stat = true : $stat = false;

...
* COS( RADIANS( $lat )) * COS( RADIANS( Longitude ) - RADIANS( $long )) ) * 3964.3 AS distance FROM Users WHERE ACOS( SIN( RADIANS( Latitude ) ) * SIN( RADIANS( $lat ) ) + COS( RADIANS( Latitude ) ) * COS( RADIANS( $lat )) * COS( RADIANS( Longitude ) - RADIANS( $long )) ) * 3964.3 < {$range} {$age_group_qry} {$users_in_qry} AND Users_ID ! = '$current_user' $gend_cond ORDER BY distance");
		...
		public function check_user_paid($ev_id, $user_id){
			
			$data = array('mtype' => 'event', 'mtype_ref' => $ev_id, 'Users_ID' => $user_id);
			$get_pay = $this->db->get_where('PayTransactions', $data);
			return $get_pay->row_array();
		}
		...
		/*	$user = $this->session->userdata('users_id');
			...
			$this->db->where(array('Ev_Rep_ID'=>$rep_id[$i], 'Users_Id'=>$user[$i]));
		...
		public function upd_ev_invite($data){
			$ev_rep_id = $data['ev_rep_id'];
			$user	   = $data['user'];
			//$ev_id	   = $data['event_id'];

			$data = array('Ev_status' => 'Accept');

			$this->db->where(array('Ev_Rep_ID' => $ev_rep_id, 'Users_Id' => $user));
			$exe_qry = $this->db->update('Ev_Inv_Status', $data);
		}
		...
		public function check_is_invited_user($data){
			...
			$data2 = array('Ev_ID' => $event_id, 'Users_Id' => $user_id);
			$get_ev_rep = $this->db->get_where('Ev_Inv_Status', $data2);
			return $get_ev_rep->result();
		}
